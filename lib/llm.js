...
async function completeChat({
  ...
  const {
    ...
    model: selectedModel
  } = liveConfig(model, { baseUrl, apiKey });
  ...
  if (jsonHint) {
    body.response_format = { type: "json_object" };
    if (shouldDisableThinkingForJson(selectedModel)) body.enable_thinking = false;
  }
  const request = async (payload) => fetch(chatCompletionsUrl(selectedBaseUrl), {
    method: "POST",
    redirect: "error",
  ...
  if (!res.ok && jsonHint && body.response_format) {
    const txt = await res.text().catch(() => "");
    if (/Structured response content must be a string|response_format|json_object|json mode is not supported|invalid input/i.test(txt)) {
...
module.exports = {
  ...
  completeChat,
  provider,
  _internals: { responseTextFromChatCompletion, requireResponseText }
};
