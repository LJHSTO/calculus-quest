...
const FunctionsLimitsTasks = require("../data/functions-limits-courseware-contracts.json");
...
function addKnowledgeUnit(units, routeVersion, chapter, module, knowledgePoint) {
  ...
  const taskContract = FunctionsLimitsTasks.knowledgePoints[unitId] || null;
...
function buildInterventionPrompt({ resolved, signal = {}, history = [] } = {}) {
  ...
  const system = [
    ...
    "taskContract 是当前课件的任务边界；只能围绕 learningTask 和已声明的 evidence 判断是否介入，不得把课件标题当作学生任务，也不得凭空创造任务。",
    "只有课件契约声明的 supports 才能成为环境动作；没有 taskContract 或契约不完整时，降级为 stay_silent 或普通响应式问答。",
  ...
  const payload = {
    learningPosition: {
      ...
      taskContract: resolved.unit.taskContract || null
    },
...
function unresolvedVisualReferenceReply({ resolved, question = "" } = {}) {
  const ref = resolved?.contextRef;
  if (resolved?.isQuiz || ref?.kind !== "viewport" || !ref.coarse) return "";
  const referencesUnlocatedObject = /点中|选中|图中|画面中|这(?:条|段|块|个).*(?:线|点|图|颜色)|(?:红|蓝|绿|黄|紫|黑|白)色(?:线|曲线|区域|点|块|部分)/.test(question);
  if (!referencesUnlocatedObject) return "";
  return "目前的引用只定位到整个画布，无法确认你指的是哪一个对象，也不能凭颜色判断变化原因。请告诉我对象的名称或公式，以及变化前后的参数值，我们再一起判断。";
}
...
function buildAssistantPrompt({
  ...
  const system = [
    ...
    "函数唯一性只约束定义域内的输入。实数平方根函数的定义域为非负实数；负数不在定义域内，不能据此说平方根不是函数或出现一输入多输出。不同输入可以有相同输出。",
    "若左右极限存在且等于同一个有限值，只是该点缺值或函数值不等于极限，这是可去间断，不是跳跃间断或振荡间断。左右有限极限不相等才是跳跃；不要把任何不连续都叫跳跃。给出具体邻近取值时先代入计算，趋近不等于已经等于。",
    "一次计算正确、一次滑块操作、有限几个点的观察或游戏通关都不能证明学生已经掌握；只能称为本次局部表现。不能用“真正掌握的标志”等说法夸大证据，也不将有限观测当作对所有输入的证明。",
    "学生的疑问不是已作出的断言。温和澄清，不用“学生说反了”等批评式开场，不引入没有必要的新概念或例子。",
    "解释近似和极限时区分一般情形与特殊情形：不能排除某些函数或取样方法使有限近似恰好等于精确值，不将收敛等同于任意一步误差都变小。",
    "建议课件操作时严格遵守 selectedContext.state 的 min/max；已提供参数名、oldValue、newValue 时准确使用原数值，不替换成猜测值。没有参数范围时不要编造可操作的数值。",
...
module.exports = {
  ...
  unresolvedVisualReferenceReply,
  responseChunks,
