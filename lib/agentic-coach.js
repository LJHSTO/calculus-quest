...
const COACH_NARRATION_TIMEOUT_MS = Math.max(
  100,
  Number(process.env.COACH_NARRATION_TIMEOUT_MS || 12000)
);
...
function recommendedAction(planResult = {}) {
  if (!planResult?.ok) return "continue";
  if (Array.isArray(planResult.remediationCandidates) && planResult.remediationCandidates.length) return "remediate";
  if (Array.isArray(planResult.skipCandidates) && planResult.skipCandidates.length) return "skip";
  if (Array.isArray(planResult.extensionCandidates) && planResult.extensionCandidates.length) return "extend";
  return "continue";
}
...
async function explain(planResult, { studentName = "同学", studyGoal, assessmentInsight, gradingFeedback, interactionEvidence, plannerInsight } = {}) {
  ...
  if (String(process.env.COACH_NARRATION_LLM_ENABLED || "").trim().toLowerCase() !== "true") {
    return {
      narration: "你可以继续学习，也可以根据下方建议选择下一步。",
      provider: "rules"
    };
  }
  ...
  if (plannerInsight?.ok) {
    const top = plannerInsight.recommendedResource || plannerInsight.rankedResourceChoices?.[0];
    extraContext.push(`\n## Planner Agent 资源建议\n策略：${plannerInsight.strategy || "current_knowledge_resource_reference"}，建议资源：${top?.title || top?.typeId || "无"}，证据：${(top?.reasonLabels || top?.reasons || []).join("、") || "无"}。这只是参考，不得替学生选择。`);
  }
  const user = [
    `学生：${studentName}`,
  ...
  try {
    ...
    const timeout = new Promise((_, reject) => {
      timer = setTimeout(() => {
        controller.abort();
        const error = new Error(`Coach narration timed out after ${COACH_NARRATION_TIMEOUT_MS}ms`);
        error.code = "COACH_NARRATION_TIMEOUT";
        reject(error);
      }, COACH_NARRATION_TIMEOUT_MS);
    });
    let out;
    try {
      out = await Promise.race([
        llm.completeChat({
          ...
          maxTokens: 500,
          model: String(
            process.env.COACH_NARRATION_MODEL
...
module.exports = { plan, explain, recommendedAction };
