...
function knowledgeCheckUnit(chapter, module, knowledgePoint, order) {
  return {
    ...
    order,
    type: "quiz",
    title: `${knowledgePoint.name} · 即时检测`,
    role: "formative_quiz",
    modality: "assessment",
    representation: "assessment",
    scenarioType: "check",
    ...
    assessedKnowledgePointIds: [knowledgePoint.id],
    isCheckpoint: true,
    isOptional: false
  };
}
...
function routeUnits(chapter = {}) {
  ...
  flattened.forEach((entry) => {
    ...
    units.push(knowledgeCheckUnit(chapter, entry.module, entry.knowledgePoint, order++));
  });
...
function validateKnowledgeGraph(route, graph, {
  rootDir = process.cwd(),
  checkResources = true,
  resourceChapterIds = null
} = {}) {
  ...
  const resourceChapterList = resourceChapterIds === null || resourceChapterIds === undefined
    ? null
    : (resourceChapterIds instanceof Set
      ? [...resourceChapterIds]
      : Array.isArray(resourceChapterIds)
        ? resourceChapterIds
        : [resourceChapterIds]);
  const resourceChapterSet = resourceChapterList === null
    ? null
    : new Set(resourceChapterList.map((id) => String(id || "").trim()).filter(Boolean));
  const resourceNodes = (graph.nodes || []).filter((node) => (
    node.kind === "unit"
    && (!resourceChapterSet || resourceChapterSet.has(node.chapterId))
  ));
  ...
  return {
    ...
    resourceChapterIds: resourceChapterSet ? [...resourceChapterSet] : null,
    missingChapters,
    unexpectedChapters,
    missingUnits,
    unexpectedUnits,
