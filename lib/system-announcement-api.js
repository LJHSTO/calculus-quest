...
function activePayload(database) {
  const currentIso = new Date().toISOString();
  return {
    announcements: listAnnouncements(database, {
      activeOnly: true,
      currentIso,
      limit: 20
    }).map(systemAnnouncements.publicAnnouncement),
...
async function handle(context) {
  ...
  if (req.method === "GET" && url.pathname === "/api/announcements") {
    sendJson(res, 200, { ok: true, ...activePayload(database) });
    return true;
  }
