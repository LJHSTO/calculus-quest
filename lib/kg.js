...
function runtimeResourceChapterIds() {
  const configured = process.env.PROACTIVE_PUBLIC_CHAPTER_ONLY;
  const publicChapterOnly = configured === undefined
    ? true
    : TRUTHY_ENV_VALUES.has(String(configured).trim().toLowerCase());
  if (!publicChapterOnly) return null;
  const chapterId = String(process.env.PROACTIVE_EXPERIMENT_CHAPTER_ID || "V14-C1").trim();
...
function aiReviewUnavailable(row = {}) {
  ...
  const feedback = String(row.ai_feedback || row.aiFeedback || "");
  return FAILED_AI_REVIEW_FEEDBACK_RE.test(feedback);
}

function load() {
  const file = path.join(process.cwd(), "data", "knowledge-graph.json");
  const routeFile = path.join(process.cwd(), "data", "multi-scene-learning-route.json");
  ...
  assertKnowledgeGraph(loadRoute(process.cwd()), graph, {
    rootDir: process.cwd(),
    resourceChapterIds: runtimeResourceChapterIds()
  });
  cache = graph;
