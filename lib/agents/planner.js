...
const RELEARN_SCENE_ORDERS = new Set([5, 11, 12]);
const EXTENSION_SCENE_ORDERS = new Set([13, 14]);

const FALLBACK_CLUSTERS = [
  { id: "diagnose-map", orders: [1, 2] },
  { id: "function-coordinate", orders: [3, 4, 5, 6] },
  { id: "slope-transfer", orders: [7, 8, 9, 11] },
  { id: "local-linear", orders: [10, 13, 14] },
  { id: "review-post", orders: [12, 15] }
];
...
function sceneMetadataByOrder(chapterId) {
  const file = path.join(process.cwd(), "resources", "open-maic", chapterId, "index.json");
  const courseware = safeJson(file);
  const map = new Map();
  (courseware?.scenes || []).forEach((scene, index) => {
...
function fallbackClusterForOrder(order) {
  return FALLBACK_CLUSTERS.find((cluster) => cluster.orders.includes(order)) || null;
}

function normalizeUnit(unit, sceneMeta = {}) {
  const order = Number(unit.order || sceneMeta.order || parseUnitId(unit.id)?.order || 0);
  const fallbackCluster = fallbackClusterForOrder(order);
  const conceptClusterId = unit.conceptClusterId || sceneMeta.conceptClusterId || fallbackCluster?.id || "";
  return {
    id: unit.id,
    chapterId: unit.chapterId || parseUnitId(unit.id)?.chapterId || "",
    order,
    title: unit.title || sceneMeta.title || unit.label || unit.id,
    type: unit.type || sceneMeta.type || "",
    role: unit.role || sceneMeta.role || "",
    modality: unit.modality || sceneMeta.modality || sceneMeta.representation || "",
    conceptClusterId,
    conceptClusterLabel: unit.conceptClusterLabel || sceneMeta.conceptClusterLabel || conceptClusterId,
    conceptClusterFocus: unit.conceptClusterFocus || sceneMeta.conceptClusterFocus || "",
    concepts: Array.isArray(unit.concepts) && unit.concepts.length
      ? unit.concepts
      : Array.isArray(sceneMeta.concepts) ? sceneMeta.concepts : [],
    representation: unit.representation || sceneMeta.representation || unit.modality || "",
    scenarioType: unit.scenarioType || sceneMeta.scenarioType || scenarioTypeFor(unit, order),
    difficultyBand: unit.difficultyBand || sceneMeta.difficultyBand || difficultyBandFor(unit, order),
    flowKind: CORE_SCENE_ORDERS.has(order) ? "core" : "adaptive"
  };
}

function scenarioTypeFor(unit, order) {
  if (unit.role === "pre_test") return "diagnose";
  if (unit.role === "post_test") return "transfer";
  if (unit.role === "formative_quiz") return "check";
  if (RELEARN_SCENE_ORDERS.has(order)) return "remediate";
  if (EXTENSION_SCENE_ORDERS.has(order)) return order === 14 ? "preview" : "extend";
  if (unit.type === "interactive") return "manipulate";
  return "explain";
}

function difficultyBandFor(unit, order) {
  if (unit.role === "pre_test") return "diagnostic";
  if (unit.role === "post_test") return "transfer";
  if (RELEARN_SCENE_ORDERS.has(order)) return "remedial";
  if (EXTENSION_SCENE_ORDERS.has(order)) return "extension";
  return "core";
}

function chapterUnits(chapterId) {
  const scenes = sceneMetadataByOrder(chapterId);
  return kg.unitsInChapter(chapterId).map((unit) => normalizeUnit(unit, scenes.get(Number(unit.order)) || {}));
}
...
function sameClusterUnits(units, current) {
  ...
  const fallback = fallbackClusterForOrder(current.order);
  if (!fallback) return units.filter((unit) => Math.abs(unit.order - current.order) <= 2);
  return units.filter((unit) => fallback.orders.includes(unit.order));
}
...
function plan({ chapterId, currentUnitId, assessment, analyticsResult, interactionEvidence, quizSummary, completedUnitIds = [] }) {
  ...
  const parsed = parseUnitId(currentUnitId);
  const current = units.find((unit) => unit.id === currentUnitId)
    || normalizeUnit({ id: currentUnitId, chapterId, order: parsed?.order || 0 }, sceneMetadataByOrder(chapterId).get(parsed?.order || 0) || {});
