...
const GRADING_MODEL =
  ...
  || "gemini-2.5-flash";
const GRADING_PROVIDER = (
  process.env.GRADING_LLM_PROVIDER
  || process.env.LLM_PROVIDER
  || "mock"
).toLowerCase();
const GRADING_BASE_URL =
  process.env.GRADING_BASE_URL
  || process.env.OPENAI_COMPATIBLE_BASE_URL
  || process.env.INNOSPARK_BASE_URL
  || "";
const GRADING_API_KEY =
...
async function gradeOne(q) {
  if (!["openai-compatible", "innospark", "openai"].includes(GRADING_PROVIDER)) {
    return {
      ...gradingIdentity(q),
      score: 0,
      isCorrect: false,
  ...
  try {
    const result = await completeChatForGrading({
      ...
      baseUrl: GRADING_BASE_URL,
      apiKey: GRADING_API_KEY
    });
    const parsed = safeParse(result.text);
    if (!parsed) {
      return { ...gradingIdentity(q), score: 0, isCorrect: false, confidence: 0, errorType: "parse_error", weakConcepts: [], feedback: `${gradingFallbackFeedback("智能评分未能解析返回内容")} 原始片段：${String(result.text || "").slice(0, 120)}`, reasoning: "", needsReview: true, provider: result.provider };
    }
    ...
    if (isGradingTimeout(err)) {
      return { ...gradingIdentity(q), score: 0, isCorrect: false, confidence: 0, errorType: "api_timeout", weakConcepts: [], feedback: gradingFallbackFeedback(`智能评分在 ${Math.ceil(GRADING_TIMEOUT_MS / 1000)} 秒内未返回`), reasoning: "", needsReview: true, provider: "timeout" };
    }
    ...
    return { ...gradingIdentity(q), score: 0, isCorrect: false, confidence: 0, errorType: "api_error", weakConcepts: [], feedback: gradingFallbackFeedback("智能评分请求暂时失败"), reasoning: "", needsReview: true, provider: "error" };
  }
}
