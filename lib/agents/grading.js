...
const GRADING_MODEL = process.env.GRADING_MODEL || process.env.PIONEER_MODEL || "pioneer/auto";
...
function buildPrompt(q) {
  const parts = [`题目：${q.questionText || q.title || "（无题目文本）"}`];
  if (q.referenceAnswer) parts.push(`参考答案：${q.referenceAnswer}`);
  if (q.rubric) parts.push(`评分标准：${q.rubric}`);
  if (q.points) parts.push(`满分：${q.points} 分`);
  if (q.concepts) parts.push(`涉及概念：${Array.isArray(q.concepts) ? q.concepts.join("、") : q.concepts}`);
...
async function gradeOne(q) {
  if (require("../llm").provider() !== "pioneer") {
    return {
      ...gradingIdentity(q),
      score: null,
      isCorrect: null,
      ...
      weakConcepts: [],
      feedback: "未启用真实大模型，简答题保留给人工复核。",
      reasoning: "LLM_PROVIDER 不是 pioneer。",
      needsReview: true,
      provider: "mock"
    };
  }
  try {
    ...
    if (!parsed) {
      return { ...gradingIdentity(q), score: null, isCorrect: null, confidence: 0, errorType: "parse_error", weakConcepts: [], feedback: `AI 评分解析失败，已转人工复核。原始片段：${String(result.text || "").slice(0, 120)}`, reasoning: "", needsReview: true, provider: result.provider };
    }
    // Reflective self-check: validate parsed fields against constraints
    const score = Math.max(0, Math.min(100, Math.round(normaliseScoreValue(parsed.score) || 0)));
    ...
    const inconsistent = (score === 0 && parsed.isCorrect === true) || (score > 80 && parsed.isCorrect === false);
    ...
    if (isGradingTimeout(err)) {
      return { ...gradingIdentity(q), score: null, isCorrect: null, confidence: 0, errorType: "api_timeout", weakConcepts: [], feedback: "AI 评分超时，已转人工复核。", reasoning: "", needsReview: true, provider: "timeout" };
    }
    return { ...gradingIdentity(q), score: null, isCorrect: null, confidence: 0, errorType: "api_error", weakConcepts: [], feedback: `评分出错：${err.message}`, reasoning: "", needsReview: true, provider: "error" };
  }
}
