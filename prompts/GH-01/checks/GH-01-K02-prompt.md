...
keyPoints 必须恰好 3 个字符串，每个包含完整题干、4 个选项、答案和解析：Q1 single 坐标含义辨析；Q2 single 用给定一次函数计算一个点；Q3 multiple 常见错误诊断。single 恰好一个正确答案；multiple 的正确项数量不固定，可以为 1、2 或 3，但至少有一个正确项和一个错误项，题干必须明确要求选择正确说法还是错误说法。正确答案位置不得形成固定规律。
