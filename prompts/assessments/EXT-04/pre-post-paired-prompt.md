...
章节名称：扩展：深度学习数学直觉
学习模块 ID：EXT-04
学习模块名称：深度学习数学直觉：从矩阵到优化
先修要求：建议完成 V14-C6 后学习。

本模块现有知识点：
- ID：EXT-04-K01
  名称：多层网络的前向传播与矩阵视角
  学习目标：能解释「多层网络的前向传播与矩阵视角」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「多层网络的前向传播与矩阵视角」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：EXT-04-K02
  名称：反向传播的完整推导直觉
  学习目标：能解释「反向传播的完整推导直觉」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「反向传播的完整推导直觉」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：EXT-04-K03
  名称：损失地形、鞍点、局部极小与逃逸策略
  学习目标：能解释「损失地形、鞍点、局部极小与逃逸策略」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「损失地形、鞍点、局部极小与逃逸策略」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：EXT-04-K04
  名称：正则化的数学
  学习目标：能解释「正则化的数学」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「正则化的数学」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：EXT-04-K05
  名称：Adam/SGD 变体
  学习目标：能解释「Adam/SGD 变体」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「Adam/SGD 变体」的符号或结论，没有把它和图像、操作或应用情境连接起来。
...
1. id="EXT-04-pre"；title="前测：深度学习数学直觉：从矩阵到优化（A卷）"；order=1。
2. id="EXT-04-post"；title="后测：深度学习数学直觉：从矩阵到优化（B卷）"；order=2。

场景与题目归属是硬约束：第一个 outline 的 keyPoints 只能依次包含 EXT-04-pre-q1 至 EXT-04-pre-q6；第二个 outline 的 keyPoints 只能依次包含 EXT-04-post-q1 至 EXT-04-post-q6。不得把 post 题放入 pre outline，不得把 pre 题放入 post outline，不得把同一道题同时放入两个 outline。整个响应中题目对象总数必须恰好为 12，每个规定 id 恰好出现一次；一个 outline 的 keyPoints 数组结束后立即关闭该 outline 对象，再开始下一个 outline 对象。
...
每个题目对象完整写出 id、type、question、options（text 除外）、answer、analysis、points、knowledgePointIds、cognitiveLevel、estimatedSteps、pairId 和 equivalence。题目 id 必须严格为 EXT-04-pre-q1 至 EXT-04-pre-q6、EXT-04-post-q1 至 EXT-04-post-q6，不得缩写、重复或增加 q6b 等变体；pairId 必须按题位严格使用 P01 至 P06，每卷各出现一次。
