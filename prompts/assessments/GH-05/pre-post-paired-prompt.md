...
章节名称：求导规则与导数应用
学习模块 ID：GH-05
学习模块名称：导数应用：单调、极值与弯曲
先修要求：已完成前序主线知识点。

本模块现有知识点：
- ID：GH-05-K01
  名称：导函数图像读法
  学习目标：能解释「导函数图像读法」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「导函数图像读法」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-05-K02
  名称：临界点与极值
  学习目标：能解释「临界点与极值」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「临界点与极值」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-05-K03
  名称：二阶变化和凹凸
  学习目标：能解释「二阶变化和凹凸」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「二阶变化和凹凸」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-05-K04
  名称：实际最值建模
  学习目标：能解释「实际最值建模」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「实际最值建模」的符号或结论，没有把它和图像、操作或应用情境连接起来。
...
1. id="GH-05-pre"；title="前测：导数应用：单调、极值与弯曲（A卷）"；order=1。
2. id="GH-05-post"；title="后测：导数应用：单调、极值与弯曲（B卷）"；order=2。

场景与题目归属是硬约束：第一个 outline 的 keyPoints 只能依次包含 GH-05-pre-q1 至 GH-05-pre-q6；第二个 outline 的 keyPoints 只能依次包含 GH-05-post-q1 至 GH-05-post-q6。不得把 post 题放入 pre outline，不得把 pre 题放入 post outline，不得把同一道题同时放入两个 outline。整个响应中题目对象总数必须恰好为 12，每个规定 id 恰好出现一次；一个 outline 的 keyPoints 数组结束后立即关闭该 outline 对象，再开始下一个 outline 对象。
...
每个题目对象完整写出 id、type、question、options（text 除外）、answer、analysis、points、knowledgePointIds、cognitiveLevel、estimatedSteps、pairId 和 equivalence。题目 id 必须严格为 GH-05-pre-q1 至 GH-05-pre-q6、GH-05-post-q1 至 GH-05-post-q6，不得缩写、重复或增加 q6b 等变体；pairId 必须按题位严格使用 P01 至 P06，每卷各出现一次。
