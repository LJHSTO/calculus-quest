...
- ID：GH-10-K01
  名称：多元链式法则
  学习目标：能解释「多元链式法则」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「多元链式法则」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-10-K02
  名称：向量值函数
  学习目标：能解释「向量值函数」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「向量值函数」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-10-K03
  名称：Jacobian 表格
  学习目标：能解释「Jacobian 表格」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「Jacobian 表格」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-10-K04
  名称：局部线性变形
  学习目标：能解释「局部线性变形」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「局部线性变形」的符号或结论，没有把它和图像、操作或应用情境连接起来。
...
1. id="GH-10-pre"；title="前测：多元链式法则与 Jacobian 实战（A卷）"；order=1。
2. id="GH-10-post"；title="后测：多元链式法则与 Jacobian 实战（B卷）"；order=2。

场景与题目归属是硬约束：第一个 outline 的 keyPoints 只能依次包含 GH-10-pre-q1 至 GH-10-pre-q6；第二个 outline 的 keyPoints 只能依次包含 GH-10-post-q1 至 GH-10-post-q6。不得把 post 题放入 pre outline，不得把 pre 题放入 post outline，不得把同一道题同时放入两个 outline。整个响应中题目对象总数必须恰好为 12，每个规定 id 恰好出现一次；一个 outline 的 keyPoints 数组结束后立即关闭该 outline 对象，再开始下一个 outline 对象。
...
每个题目对象完整写出 id、type、question、options（text 除外）、answer、analysis、points、knowledgePointIds、cognitiveLevel、estimatedSteps、pairId 和 equivalence。题目 id 必须严格为 GH-10-pre-q1 至 GH-10-pre-q6、GH-10-post-q1 至 GH-10-post-q6，不得缩写、重复或增加 q6b 等变体；pairId 必须按题位严格使用 P01 至 P06，每卷各出现一次。
