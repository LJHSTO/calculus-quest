...
场景与题目归属是硬约束：第一个 outline 的 keyPoints 只能依次包含 GH-10-pre-q1 至 GH-10-pre-q10；第二个 outline 的 keyPoints 只能依次包含 GH-10-post-q1 至 GH-10-post-q10。不得把 post 题放入 pre outline，不得把 pre 题放入 post outline，不得把同一道题同时放入两个 outline。整个响应中题目对象总数必须恰好为 20，每个规定 id 恰好出现一次；一个 outline 的 keyPoints 数组结束后立即关闭该 outline 对象，再开始下一个 outline 对象。
...
每个题目对象完整写出 id、type、question、options（text 除外）、answer、analysis、points、knowledgePointIds、cognitiveLevel、estimatedSteps、pairId 和 equivalence。题目 id 必须严格为 GH-10-pre-q1 至 GH-10-pre-q10、GH-10-post-q1 至 GH-10-post-q10，不得缩写、重复或增加 q10b 等变体；pairId 必须按题位严格使用 P01 至 P10，每卷各出现一次。
