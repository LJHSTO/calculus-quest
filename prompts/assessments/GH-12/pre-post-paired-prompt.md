...
- ID：GH-12-K01
  名称：目标函数地形
  学习目标：能解释「目标函数地形」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「目标函数地形」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-12-K02
  名称：负梯度方向
  学习目标：能解释「负梯度方向」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「负梯度方向」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-12-K03
  名称：步长和收敛
  学习目标：能解释「步长和收敛」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「步长和收敛」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-12-K04
  名称：迭代路径与停止信号
  学习目标：能解释「迭代路径与停止信号」的核心含义，并用交互证据说明自己的判断。
...
1. id="GH-12-pre"；title="前测：无约束优化与梯度下降基础（A卷）"；order=1。
2. id="GH-12-post"；title="后测：无约束优化与梯度下降基础（B卷）"；order=2。

场景与题目归属是硬约束：第一个 outline 的 keyPoints 只能依次包含 GH-12-pre-q1 至 GH-12-pre-q6；第二个 outline 的 keyPoints 只能依次包含 GH-12-post-q1 至 GH-12-post-q6。不得把 post 题放入 pre outline，不得把 pre 题放入 post outline，不得把同一道题同时放入两个 outline。整个响应中题目对象总数必须恰好为 12，每个规定 id 恰好出现一次；一个 outline 的 keyPoints 数组结束后立即关闭该 outline 对象，再开始下一个 outline 对象。
