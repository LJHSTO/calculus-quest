...
章节名称：扩展：微分方程直觉
学习模块 ID：EXT-01
学习模块名称：微分方程直觉入门：从变化率到函数
先修要求：建议完成 V14-C3 后学习。

本模块现有知识点：
- ID：EXT-01-K01
  名称：什么是微分方程
  学习目标：能解释「什么是微分方程」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「什么是微分方程」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：EXT-01-K02
  名称：可分离变量与一阶线性方程
  学习目标：能解释「可分离变量与一阶线性方程」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「可分离变量与一阶线性方程」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：EXT-01-K03
  名称：指数增长与衰减模型
  学习目标：能解释「指数增长与衰减模型」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「指数增长与衰减模型」的符号或结论，没有把它和图像、操作或应用情境连接起来。
...
1. id="EXT-01-pre"；title="前测：微分方程直觉入门：从变化率到函数（A卷）"；order=1。
2. id="EXT-01-post"；title="后测：微分方程直觉入门：从变化率到函数（B卷）"；order=2。

场景与题目归属是硬约束：第一个 outline 的 keyPoints 只能依次包含 EXT-01-pre-q1 至 EXT-01-pre-q6；第二个 outline 的 keyPoints 只能依次包含 EXT-01-post-q1 至 EXT-01-post-q6。不得把 post 题放入 pre outline，不得把 pre 题放入 post outline，不得把同一道题同时放入两个 outline。整个响应中题目对象总数必须恰好为 12，每个规定 id 恰好出现一次；一个 outline 的 keyPoints 数组结束后立即关闭该 outline 对象，再开始下一个 outline 对象。
...
每个题目对象完整写出 id、type、question、options（text 除外）、answer、analysis、points、knowledgePointIds、cognitiveLevel、estimatedSteps、pairId 和 equivalence。题目 id 必须严格为 EXT-01-pre-q1 至 EXT-01-pre-q6、EXT-01-post-q1 至 EXT-01-post-q6，不得缩写、重复或增加 q6b 等变体；pairId 必须按题位严格使用 P01 至 P06，每卷各出现一次。
