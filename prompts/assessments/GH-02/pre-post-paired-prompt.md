...
- P01：GH-02-K01；single；给出完整的双侧数表，从数值趋势读取趋近值；A/B 使用相同行列数、完全相同的小数位数分布、相同正负号复杂度和相同判断步骤；8 分。
- P02：GH-02-K01；single；给出完整的双侧数表，辨析趋近值不必等于表中任何一个数；A/B 使用相同行列数、完全相同的小数位数分布和相同正负号复杂度；8 分。
