...
知识点 ID：GH-02-K03
知识点名称：连续就是不跳断
知识点目标：能解释「连续就是不跳断」的核心含义，并用交互证据说明自己的判断。
...
允许内容：基于已经明确给出的左极限、右极限和该点函数值，判断三者是否相等，并据此判断是否连续；可以诊断学生在连续性条件判断中的典型错误。

禁止内容：不得引入介值定理、零点存在性、去心邻域、标准极限、导数或需要额外代数技巧的函数。
...
只考查“连续就是不跳断”，不得混入同模块其他知识点、后续知识点或额外内容。参考正式数学教材课后练习题的严谨性、条件完整性和推理结构，但面向高中生控制符号密度、阅读量和计算长度。题意必须能够独立理解，数据适合手算，不使用竞赛技巧，每题只测量一个明确目标。

【输出前静默检查】

逐题重新计算并确认：只输出一个 quiz；keyPoints 恰好有 2 个完整题目字符串；Q1 是 10 分 single 核心题且 adaptiveRole="core"；Q2 是 10 分 multiple 诊断题且 adaptiveRole="diagnostic"；Q2 只在 Q1 错误时出现；两题只考查 GH-02-K03，没有场景或图片依赖，答案、选项与解析完全一致。
```
