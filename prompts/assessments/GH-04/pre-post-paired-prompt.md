...
- ID：GH-04-K01
  名称：幂函数求导
  学习目标：能解释「幂函数求导」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「幂函数求导」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-04-K02
  名称：和差积商规则
  学习目标：能解释「和差积商规则」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「和差积商规则」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-04-K03
  名称：链式法则
  学习目标：能解释「链式法则」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「链式法则」的符号或结论，没有把它和图像、操作或应用情境连接起来。
- ID：GH-04-K04
  名称：常见函数变化速度
  学习目标：能解释「常见函数变化速度」的核心含义，并用交互证据说明自己的判断。
  常见误解：只记住「常见函数变化速度」的符号或结论，没有把它和图像、操作或应用情境连接起来。
...
1. id="GH-04-pre"；title="前测：常用求导规则与函数组合（A卷）"；order=1。
2. id="GH-04-post"；title="后测：常用求导规则与函数组合（B卷）"；order=2。

场景与题目归属是硬约束：第一个 outline 的 keyPoints 只能依次包含 GH-04-pre-q1 至 GH-04-pre-q6；第二个 outline 的 keyPoints 只能依次包含 GH-04-post-q1 至 GH-04-post-q6。不得把 post 题放入 pre outline，不得把 pre 题放入 post outline，不得把同一道题同时放入两个 outline。整个响应中题目对象总数必须恰好为 12，每个规定 id 恰好出现一次；一个 outline 的 keyPoints 数组结束后立即关闭该 outline 对象，再开始下一个 outline 对象。
...
每个题目对象完整写出 id、type、question、options（text 除外）、answer、analysis、points、knowledgePointIds、cognitiveLevel、estimatedSteps、pairId 和 equivalence。题目 id 必须严格为 GH-04-pre-q1 至 GH-04-pre-q6、GH-04-post-q1 至 GH-04-post-q6，不得缩写、重复或增加 q6b 等变体；pairId 必须按题位严格使用 P01 至 P06，每卷各出现一次。
