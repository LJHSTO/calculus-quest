...
const agenticPath = read("app/main/agentic-path.js");
const narration = read("app/main/narration.js");
const quiz = read("app/main/quiz.js");
...
assert.doesNotMatch(bootstrap, /nextBtn\.disabled\s*=\s*needsSceneChoice/);
assert.match(bootstrap, /completeAndAdvanceCurrentUnit/);
...
assert.match(renderLearning, /function focusKnowledgeSceneChoicePanel/);
assert.match(renderLearning, /id="knowledge-scene-panel"/);
...
assert.match(
  styles,
  /\.multi-scene-slide-fullscreen-stage:fullscreen[\s\S]*\.multi-scene-required-slide[\s\S]*width:\s*min\(100vw/
);
assert.match(renderLearning, /data-resource-fullscreen-target/);
assert.match(renderLearning, /讲解页全屏/);
assert.match(events, /resourceFullscreenTargetForButton/);
assert.match(narration, /function resourceFullscreenTargetForButton/);
...
assert.match(agenticPath, /function agenticUpdateReviewChoicesBulk/);
assert.match(agenticPath, /data-agentic-review-bulk/);
assert.match(agenticPath, /function agenticUpdateKnowledgeChoicesBulk/);
assert.match(agenticPath, /data-agentic-knowledge-bulk/);
assert.match(agenticPath, /deferredExtensionPlan/);
assert.match(agenticPath, /deferredReviewPlan/);
assert.match(events, /data-agentic-review-bulk/);
assert.match(events, /data-agentic-knowledge-bulk/);
assert.match(navigation, /function completeAndAdvanceCurrentUnit/);
assert.match(quiz, /agenticPreviousUnlockedUnitBefore/);
assert.match(quiz, /agenticNextUnlockedUnitAfter/);
