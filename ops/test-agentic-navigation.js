...
reset({
  ...
  state: {
    ...
    pendingKnowledgeTransition: {
      knowledgeUnitId: "C1-k1",
      formativeUnitId: "C1-formative",
      targetUnitId: "C1-k2"
    },
    agenticPath: {
      unlocked: ["C1-pre", "C1-k1"],
...
context.ensureAgenticPath();
assert.equal(
  context.state.agenticPath.unlocked.includes("C1-formative"),
  false,
  "a pending knowledge transition must not silently unlock the optional check"
);
assert.equal(
  context.state.agenticPath.unlocked.includes("C1-k2"),
  false,
  "a pending knowledge transition must not silently unlock the continue target"
);
