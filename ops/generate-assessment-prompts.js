...
逐题重新计算并确认：两个 quiz 的 keyPoints 均恰好有 6 个完整题目字符串，不含纯考查目标摘要；每题 points=10，两卷各 6 题且 points 求和都严格等于 60；每个 pairId 在 A/B 卷各出现一次；配对题的呈现方式、先备运算、解题步骤与实际难度一致；所有知识点至少覆盖一次；所有 knowledgePointIds 来自上方清单；没有未列出的定理或标准极限；single 只有一个正确答案；multiple 有 1 至 3 个正确项且配对数量一致；答案、选项与解析一致；没有缺失条件、无关数据、图片依赖、自我纠错文字或新增知识点。
\`\`\`
`;
}
