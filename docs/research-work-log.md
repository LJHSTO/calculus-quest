...
### 2026-07-22：学习路径边界、历史状态兼容与讲解页独立全屏

- 目标：修正“上一节、完成本节、复习并跳到下一节”在课程首尾、章末推荐、主线回看和扩展学习中的跳转边界；保证回看与扩展可按任意顺序完成且不会互相覆盖；让讲解页和互动课件分别独立全屏，不把四场景选择器带入全屏。
- 路径状态：新增真实学习证据迁移，仅用已完成单元、已提交测验、答题记录和测验尝试回填历史章节门槛；没有学习证据的未来章节误解锁会被清理，已有后续章节证据的旧用户保留原位置。扩展章未完成时阻止进入后续主章，扩展前测和形成测验的“继续”只在扩展章内前进，只有扩展后测及其回看完成后才释放主线恢复点。
- 导航与按钮：课程第一节不显示“上一节”；已完成中间单元显示“复习并跳到下一节”；跨章目标显示“进入下一章”；课程最终完成后显示禁用的“课程已完成”。测验提交后先等待或确认学习建议，章节按钮、直接单元链接、顶部按钮和底部按钮统一使用同一套可达性判断。
- 自主选择：未选择互动场景时，顶部和底部按钮可点击并滚动聚焦“四场景选择”模块；回看、补学和仍然跳过保留一键批量设置；回看与扩展计划使用延迟计划分别保留。
- 全屏与桌面布局：互动“课件全屏”只全屏 iframe 舞台；“讲解页全屏”只全屏 16:9 slide 画布并提供退出按钮。桌面 `1440×900` 无页面横向溢出，章节栏为固定悬浮抽屉；`1280×720` 下互动 iframe 和讲解 slide 均实际占满 `1280×720`，选择器不在全屏元素中。
- 历史恢复浏览器验证：使用随机临时端口、隔离临时数据库和两个全新浏览器会话。第一个会话完成前测、选择全部知识点、选择“交互模拟”并完成第一知识点；第二个会话重新登录后恢复相同当前位置、已完成记录和互动场景，未使用重置。
- 课件核验：逐文件比较 GH-01 至 GH-14、EXT-01 和 EXT-02 最终包的 `manifest.json`、`interactive/` 和 `audio/` SHA-256，共 1913 个包内运行文件，缺失 0、不一致 0。GH-04 额外保留 4 个路线依赖的派生三维文件；EXT-03 至 EXT-05 按要求沿用旧版。
- 数据保护：只读比较正式 `data/calculus-quest.db` 与 `data/calculus-quest.before-courseware-route-audit-20260722-151322.db`，所有受保护表零减行；当前仍为 7 用户、28 会话、289 测验、5 反馈、58 条 Agent 决策和 146 条交互证据。正式本地服务 PID 32620、`http://127.0.0.1:8765/` 未停止，浏览器验证未连接正式数据库。
- 自动验证：19 个 `ops/test-*.js` 全部通过；56 个 JavaScript 文件通过 `node --check`；`npm run kg:test`、`flow:test`、`slide:test`、`katex:test` 通过；修改文件通过严格 UTF-8 和 `git diff --check`；`npm audit --omit=dev` 为 0。
- 失败/警告：Chromium 仍提示同源课件 iframe 同时使用 `allow-scripts` 与 `allow-same-origin` 的既有 sandbox 警告，主应用 0 个 JavaScript error。安全策略不允许删除仓库外的临时 smoke 文件夹，临时服务已停止，该目录不会进入 Git。
- 是否真实调用 LLM（provider/model）：否；浏览器验证使用默认 mock 路径。
- 是否修改源 `.maic.zip`：否；仅对最终包做只读哈希核验。
- Git 提交：本条与本次修复一起精确提交，最终提交号见 Git 历史。
- 剩余风险与下一步：GitHub 推送不会自动部署生产站点。服务器管理员仍需先备份仓库外生产数据库，快进拉取后以 `BASE_PATH=/calculus_quest` 重启，并用历史测试账号验证恢复；禁止用真实学生账号点击重置做 smoke。

### 2026-07-22：讲解页富文本兼容与 GH-03 缺图补全

- 目标：修复 GH-04“和差积商规则”表格把 `<span style=...>` 当普通文字显示的问题；排查讲解页图片空白；确认 GH-04 四个三维场景是否真实可用。
- 基线提交/课件版本：`a415df7`；GH-04 核验包为 `常用求导规则与函数组合__innospark_gpt-5.4__20260719T001402Z.maic.zip`。
- 根因：GH-04 最新包把乘法法则的红色公式存放在表格单元格 `text` 字段内，Calculus Quest 对普通文本框支持富文本，但表格单元格无条件执行 `escapeHtml`，因此标签原样显示。GH-03 第 2 页不是资源路径错误，源清单只有“图 5.3”图注，没有任何 `image` 元素，包内截图同样为空白。
- 修改文件：`app/main/render-learning.js` 让文本框与表格单元格共用富文本/公式渲染；`data/multi-scene-learning-route.json` 和 GH-03 活动 `manifest.json` 增加割线图元素；新增 `media/gh03-average-rate-secant.svg`，依据教材 PDF 第 141 页图 5.3 的数学关系重绘红色函数曲线、蓝色割线及 `Δx/Δy`；`ops/test-slide-rendering.js` 增加真实富文本回归、图注无图检查和图片文件存在性检查。
- 课件全量审计：19 个活动清单中，只有 GH-04 这一处表格单元格含 HTML；只有 GH-03 第 2 页存在“有图注但无图片元素”。路线中的其余 11 张讲解页图片均能解析到现有文件。
- GH-04 三维场景：4 个“空间视角”HTML 与最新修复包逐文件 SHA-256 一致，均返回 HTTP 200。它们使用 CSS 透视模型而非 `<canvas>`：浏览器中每页都有 3 层模型、3 条空间轴和 2 个滑块；拖动水平旋转后 `matrix3d` 实际变化，控制台 0 error。
- 浏览器验证：桌面端 `1280×720` 使用真实播放器渲染。GH-04 单元格正文为 `乘法法则：(fg)' = f'g + fg'`，内部红色为 `rgb(231, 76, 60)`，页面不再出现字面量 `<span>`；GH-03 新图实际显示约 `399×235`，图注、公式、曲线和测量标记完整，无横向溢出。
- 自动验证：19 个 `ops/test-*.js` 全部通过；61 个 JavaScript 文件通过 `node --check`；`kg:build`、`kg:test`、`flow:test`、`slide:test`、`katex:test` 通过；知识图谱保持 11 章、105 个运行单元、430 条边和 288 个互动资源；`npm audit --omit=dev` 为 0。
- 数据保护：只读比较 `data/calculus-quest.before-courseware-route-audit-20260722-151322.db` 与当前正式数据库，用户、会话、测验、事件、快照、反馈、Agent 决策和交互证据 8 张表全部零减行。未修改数据库结构、历史记录键、章节/知识点 ID 或推荐逻辑。
- 是否真实调用 LLM（provider/model）：否。
- 是否修改源 `.maic.zip`：否。GH-03 当前修复位于可部署的只读派生资源；以后若重新导入原包，必须先在 OpenMAIC 源课件中补回该图，避免覆盖。
...
### 2026-07-23：“知点”入口、课件聚焦与移动端学习体验优化

- 目标：把“知点”从固定侧栏完善为不干扰学习操作的上下文入口；让本章路径可收起并在打开知点时自动让出空间；提升 Slide、Quiz 和互动课件中可选对象的辨识度；把最近操作改成学生能看懂的中文场景、组件和值变化；优化手机端面板密度、触屏提示和焦点行为。
- 基线提交/课件版本：`f623660`；当前分支 `codex/recover-worktree-20260712`。课程事实源、章节/知识点/测验 ID、288 个课件资源和源 `.maic.zip` 均未修改。
- 入口与布局：知点入口可纵向拖动、左右停靠并持久化位置，可收成贴边知点针；贴边状态保留可访问名称“打开知点”。本章路径新增独立收起按钮并持久化状态；打开知点会自动收起本章路径和章节抽屉。宽屏保留右侧停靠能力，普通桌面使用非模态侧栏，手机使用底部面板。
- 文案与视觉：学生动作统一为“聚焦课件内容”“围绕这里提问”“带入问题”，选择提示改为“选择想深入的一处”；不再使用“指着问”“指着课件问明白”“操作回声”等旧文案。面板使用深墨蓝、知点青和薄荷高亮，保留知点针作为上下文对应标记；快捷问题、上下文卡、输入区和移动端安全区重新排版。
- 课件选择：Slide 文字按真实字形范围、连线按笔画、图形按路径命中，空白处才回退到整张讲解画面；手机选择态使用更强的青色边界和文字底色。桌面悬停约 120ms 后显示“对象类型 · 中文标签”预览。整块 `.resource-body` 不再作为候选，避免把讲解页、全屏按钮和语音包一起选中。iframe 候选收口到明确控件、图片、SVG、Canvas、表格等语义对象，不再因任意 `id` 误选容器。
- 最近操作：桥只在滑块、数字输入和下拉框的有意义提交后记录一次变化。侧栏显示具体课件场景与中文组件，例如“拖动实验 · 输入值 x”，变化显示为“从 0 调整为 0.5”；清理 emoji、装饰符号和技术 ID 前缀。互动 iframe URL 增加 `cqContextBridge=20260723-v1`，避免旧桥脚本被课件 HTML 的长缓存继续使用。
- 移动端与可访问性：`390×844` 下快捷问题保留横向浏览但隐藏原生滚动条，以渐隐提示仍有后续选项；上下文存在时消息区可安全收缩，不再把快捷问题挤出可视区域，面板无整体横向溢出。触屏提示改为“轻点已标示的内容完成选择”；打开面板或选中对象后焦点落在侧栏容器，不自动弹出软键盘，桌面精确指针仍自动聚焦输入框。刷新恢复已打开状态时同步 `aria-hidden` 和 `aria-expanded`。
- 浏览器验证：隔离服务 `http://127.0.0.1:8798/?qa=knowledge-focus-v1`，PID `17648`，数据库 `tmp/knowledge-focus-qa-20260723/qa.db`；正式 `8765` 服务 PID `11144` 未停止。桌面 `1440×1000` 验证打开知点自动收起两级路径、Slide 候选与悬停标签、iframe 控件候选、入口贴边和可访问名称；手机 `390×844` 验证打开、聚焦、单次选择、面板回弹、快捷问题、输入区和焦点。项目控制台 0 error、0 warning。
- 浏览器证据：手机底部面板 `.playwright-cli/page-2026-07-23T04-38-01-744Z.png`；手机 Slide 强候选 `.playwright-cli/page-2026-07-23T04-42-51-224Z.png`；手机选中后面板 `.playwright-cli/page-2026-07-23T04-44-16-300Z.png`；桌面悬停标签 `.playwright-cli/page-2026-07-23T04-48-23-654Z.png`。这些文件仅作本地 QA 证据，不纳入 Git。
- 运行命令：`npm run assistant:test`；逐个运行全部 `ops/test-*.js`；`node --check server.js`、`app/main/*.js` 和 `lib/**/*.js`；严格 UTF-8 解码；`git diff --check`；Playwright CLI 的 `snapshot`、`resize`、`hover`、`click`、`screenshot` 和控制台检查。
- 结果：30/30 个 `ops/test-*.js` 通过；`server.js`、`app/main/*.js` 和 `lib/**/*.js` 共 39 个文件通过 `node --check`；新增静态 UI 回归禁止旧文案、无名贴边入口、整块资源候选和移动端滚动/焦点问题回归。相关文本严格 UTF-8 解码和差异检查通过。
- 失败/警告：首次手机验证发现快捷问题只剩滚动条、整块课件会带入语音区、选中后自动聚焦输入框会在真机弹出软键盘；均已修复并复测。最终连续 API 回归时 `test-feedback-api.js` 曾一次以 Windows 快速失败码 `0xC0000409` 退出，无 JavaScript 断言；隔离连续 10 次和原 9 项连续序列复跑全部通过，未复现且未发现残留测试服务，因此按一次性 Windows/Node 进程异常记录，没有为此修改业务代码。Canvas/3D 首版仍只提供诚实的粗粒度画面定位，未新增完整语义识别。
- 是否真实调用 LLM（provider/model）：否；隔离服务使用现有本地引导流程，本轮只验证交互、上下文和界面。
- 是否修改源 `.maic.zip`：否；只修改平台代码和运行时桥接，不改写磁盘中的源包或派生课件 HTML。
...
### 2026-07-23：“知点”学习工作台、划线笔记、对话管理与硅基流动接入收口

- 目标：不照搬参考截图，在 Calculus Quest 的 Quiz、Slide 与四类互动场景中完成“围绕眼前内容提问”的桌面学习工作台；划线笔记保持为选区旁的独立动作，不占用对话工作区；历史对话按当前学习位置管理；为每个用户设置北京时间自然日额度。
- 交互与布局：知点入口采用可拖动的悬浮入口，悬浮时展示用途、点击后打开可拖动窗口；打开知点自动收起章节与本章路径。章节和路径合并为 68px 纵向控制面，修复旧网格定位导致路径按钮横向溢出并遮挡课件标题的问题。播放器通过统一的 `ResizeObserver` 同步 Quiz、Slide 和互动 iframe 尺寸，路径收起后继续使用扩展画布。
- 上下文与笔记：文字/公式直接划选后出现“问知点 / 记一笔”，对象使用单次“选取课件焦点”，互动操作使用中文场景与组件变化。四种划线颜色、编辑、删除和刷新恢复均由本地版本化存储完成；笔记编辑器在选区附近出现，不加入知点对话框。iframe 通过同源桥传递选区、对象、最近操作、笔记与宿主尺寸，选择动作不会误触滑块或 Quiz 选项。
- 对话与兼容：对话按知识点或 Quiz 学习位置隔离，历史页只展示会话摘要，避免把全部消息堆成一条长滚动。新增幂等升级迁移，把旧版没有 `conversation_id` 的知点消息按用户与 thread 归入可见的“历史对话”，标题取首个学生问题，旧消息内容原样保留。修复恢复页面时课程 ID 先于完整课程对象就绪造成的同步竞态；支持状态从未就绪变为可提问时会重新加载真实 provider、额度与历史。
- 模型与安全：本机忽略的 `.env` 配置为 `openai-compatible / https://api.siliconflow.cn/v1`，知点专用模型为 `zai-org/GLM-4.5-Air`，其他模型任务保留 `zai-org/GLM-5.2`；API Key 未输出、未写入日志、未纳入 Git。每日额度默认 48 次，按用户隔离、北京时间计日，仅真实模型请求计数。OpenAI-compatible 请求禁止重定向，Quiz 未提交时继续由服务端执行不泄题策略。
- 真实 API 证据：隔离服务 `http://127.0.0.1:8766/` 使用 `tmp/knowledge-assistant-browser.db`。真实提问“请严谨说明函数中输入与输出的关系，并解释为什么这不叫一一对应”返回了函数与单射的正确区分，界面标记为“AI 助教”，额度 `48 → 47`。刷新后请求 `assistant/conversations` 与 `assistant/history` 均为 200，历史、provider 和额度正确恢复。
- OpenMAIC v0.3.1 参考边界：吸收 RuntimeStore 会话/追加记录、聊天会话持久化、Quiz 防泄题、稳定 ID、`postMessage` 合同和多会话历史等架构启发；不复制编辑器的框选、拖拽、JSON Patch 或课件修改能力。Calculus Quest 继续只读播放派生资源，`ContextRef`、服务端可信解析和 Quiz 策略与视图解耦，未来前端重写只需替换视图和选择 adapter。
- 浏览器验证：桌面实景确认悬浮入口、纵向章节/路径控制面、窗口拖动、固定输入区、Slide 候选、iframe 操作、真实回答、额度和历史摘要。刷新恢复后控制台 0 error、0 warning（仅 Chromium 原有多表单 verbose 提示）。最终截图为 `output/playwright/knowledge-assistant-final-20260723.png` 和 `output/playwright/knowledge-assistant-history-20260723.png`，仅作本地 QA 证据，不纳入 Git。
- 自动验证：`npm run assistant:test`、`path:test`、`slide:test`、`auth:test`、`node ops/test-subpath-deployment.js`、`node ops/test-static-security.js` 全部通过；`npm run db:check -- --db tmp/knowledge-assistant-browser.db` 通过。首次直接运行 `npm run db:check` 因脚本要求 `--db` 参数退出，补充隔离库参数后通过。`server.js`、`db.js`、`app/main/*.js`、`lib/**/*.js` 及相关测试共 47 个文件通过 `node --check`；19 个相关文本严格 UTF-8 解码通过；`git diff --check` 通过。
- 是否修改源 `.maic.zip`：否。没有修改 OpenMAIC 源课件、语音包或课件内容，只修改平台视图、桥接、上下文协议和服务端问答能力。
- Git 提交：尚未暂存或提交；最终交付前只审查本功能相关路径，绝不使用 `git add .`。
- 剩余边界：首版笔记仅保存在当前浏览器，Canvas/3D 无语义桥时仍诚实标记为粗粒度定位。正式部署需要在服务器配置独立持久数据库与同名环境变量，并在真实 `/calculus_quest/` 反向代理路径再做一次登录、提问和额度 smoke。

### 2026-07-24：知点 30 次额度、草稿会话与正式 8765 同步

- 目标：将知点每日最大提问次数从 48 调整为 30；点击“创建新对话”时不再立即写入空历史，只有生成完整助教回复后才保存；停止无用测试端口并将最新后端同步到正式本地 `8765`。
- 会话状态：前端“新建对话”改为纯本地草稿，清空当前消息并提示“收到助教回复后才会保存”，不再调用创建会话接口。服务端无 `conversationId` 时只构造未落库的会话记录，先完成模型生成，再通过 `db.saveLearningAssistantTurn()` 在一个事务中同时写入会话、学生问题和助教回复。数据库模块移除可单独写入会话、消息和更新时间的浅层接口，避免未来重新产生空会话或半截问答。
- 兼容接口：`POST /api/learning/assistant/conversations` 保留上下文校验，但只返回 `draft=true / conversation=null`，不持久化数据；缓存标识更新为 `20260724-knowledge-workspace-v5`，确保旧浏览器刷新后取得草稿语义。继续追问时必须携带现有 `conversationId`；缺少 ID 明确表示开始一段新对话，不再隐式续接最近历史。
- 额度：`server.js`、知点界面默认状态、`.env.example` 与本机忽略的 `.env` 统一设置 `LEARNING_ASSISTANT_DAILY_QUOTA=30`。既有每日使用记录不重置；若当天已使用超过 30 次，剩余额度按 0 处理。
- TDD 证据：修改前 API 测试以 `201 !== 200` 失败，证明点击创建仍持久化空会话；静态 UI 测试因仍存在创建会话 POST 请求失败。修改后测试确认草稿阶段历史为 0，首次完整回复后出现 1 段两消息会话，第二次草稿回复形成独立会话，非法会话与额度耗尽请求不创建新历史。
- 浏览器验证：隔离测试服务 `8766` 使用 `tmp/knowledge-assistant-browser.db`。新用户初始额度显示 30；点击“＋”并打开历史仍显示“这里还没有历史对话”；真实硅基流动回复完成后额度变为 29，历史出现一条“2 条消息”的会话。未提交 Quiz 的回答没有给出答案；浏览器控制台 0 error、0 warning，仅保留 Chromium 原有多表单 verbose 提示。证据截图为 `output/playwright/knowledge-assistant-draft-history-20260724.png`，不纳入 Git。
- 自动验证：`npm run assistant:test`、`path:test`、`slide:test`、`auth:test`、`node ops/test-subpath-deployment.js` 与 `node ops/test-static-security.js` 全部通过。最终语法、严格 UTF-8 和差异检查见本条交付验证。
- 正式数据库保护：确认旧 `8765` PID `11144` 与 `tmp/ui-review-8765.db.lock` 一致后停止服务。停机后备份为 `tmp/ui-review-8765.before-knowledge-draft-v5-20260724-095716.db`，长度 `73043968` 字节，源与备份 SHA-256 均为 `C412E5EDCFAE91990585A8E1F8CA39E08678B6D9B5C527B86372A8072002D035`。发布检查保留 10 用户、36 会话、119 条测验、11809 条事件、857 个快照、2 条反馈、22 条 Agent 决策和 69 条交互证据快照。
- 正式服务：最新代码以原数据库路径在 `http://127.0.0.1:8765/` 重启，PID `31564`，健康检查和数据库锁一致；首页与知点脚本均返回 200，首页引用 v5 资源，脚本默认额度为 30 且不再发送空会话创建请求。`8766` 和残留的 `8876` 测试服务已关闭；OpenMAIC 的 `3000` 端口属于另一项目，未处理。
- 是否真实调用 LLM（provider/model）：是；隔离浏览器使用 `openai-compatible / zai-org/GLM-4.5-Air`，真实回答成功并消耗 1 次额度。API Key 未输出、未写入日志或 Git。
- 是否修改源 `.maic.zip`：否。
- Git 提交：尚未暂存或提交；`.env`、数据库备份、测试截图和测试数据库均保持忽略。
- 剩余边界：模型请求目前在调用前消耗额度，即使随后回退为本地引导也会计为一次尝试；是否改为“仅真实回复成功后扣额度”应作为独立产品规则决定。划线笔记仍只保存在当前浏览器，跨设备同步尚未实现。

### 2026-07-24：Quiz 提交后解锁知点与按需学习辅助

- 目标：修复未提交 Quiz 时真实模型仍可能直接算出答案的问题；把知点改为只有服务端确认测验已提交后才能使用；“回答依据”和“理解自检”只在问题确有需要时出现，不作为每轮固定尾巴。
- 根因：客户端和服务端虽然都计算了 `quizSubmitted`，但此前只把它传入提示词与输出过滤器，没有把它作为调用模型的授权条件。模型回答“是多少”时可以用自然语言直接完成计算，证明提示词和正则过滤不能充当测验权限边界。
- 服务端门控：`/api/learning/assistant/ask` 在可信 route 解析出 Quiz、并查询数据库确认没有该单元测验结果后，立即返回 `403 / assistant_quiz_locked_until_submit`。拒绝发生在速率限制、每日额度、会话构造、历史读取和模型调用之前，因此不扣 30 次额度、不创建草稿或消息。会话列表与历史接口采用同一门控，修复前遗留的未提交 Quiz 对话也不能在提交前重新打开。
- 前端门控：悬浮入口仍保留可发现性，但改为琥珀色锁定态和“提交测验后解锁”；提交前不能打开面板、发送问题、创建会话、启动对象焦点选择、使用选区提问或显示快捷问题。独立划线笔记不调用 AI，仍可正常使用。测验提交并写入 `state.submittedQuizzes` 后，入口原地恢复为“打开知点”，输入框、课件焦点与 Quiz 复盘快捷问题同步解锁。
- 按需辅助：真实模型提示不再要求每个回答都固定追加课件动作。只有学生追问出处、依据或课件位置时才说明当前上下文依据；没有可靠来源必须明说，不能虚构引用。只有概念理解、解题过程、错因或验证类问题才补充简短自检或观察动作；符号读法、导航和简单澄清保持简洁。首版没有新增常驻的“回答溯源”或“理解自检”卡片。
- TDD 证据：修复前 API 用例稳定失败为 `200 !== 403`，前端静态用例找不到锁定判断，未提交快捷问题仍返回四个提示。修复后测试覆盖提交前 `403`、额度不变、历史不可见，以及完成权威 Quiz 提交后同一提问接口返回 `quiz_review` 并正常保存回复。
- 浏览器验证：隔离服务 `http://127.0.0.1:8766/` 使用 `LLM_PROVIDER=mock` 和独立测试数据库。真实前测页面提交前的入口在辅助功能树中为不可用，`aria-disabled=true`、面板关闭且显示“提交测验后解锁”；10 道题正常作答、提交成功后入口自动变为“打开知点”，面板输入框、焦点选择和“给我完整解析”等复盘问题可用，实际提问得到回复。项目控制台 0 error、0 warning。隔离浏览器和 `8766` 已关闭。
- 自动验证：`npm run assistant:test`、`path:test`、`slide:test`、`auth:test`，以及 `test-subpath-deployment.js`、`test-static-security.js`、`test-course-assessment-security.js`、`test-client-content-security.js` 全部通过；首页在正式 `8765` 返回 `20260724-knowledge-workspace-v6` 资源版本。
- 正式数据库与服务：重启前脚本确认旧监听为 PID `31564 / node server.js 8765`。数据库备份为 `tmp/ui-review-8765.before-restart-20260724-103459.db`，备份与 `tmp/ui-review-8765.db` 的 SHA-256 均为 `FE82FA6B0613F78E5AE6A67A7592F7115DEEEA091F0F78C79931EB3108C8F428`。新服务在 `http://127.0.0.1:8765/` 以 PID `11976` 运行，健康检查通过。
- 失败/警告：安全策略拒绝删除隔离测试数据库，`tmp/quiz-lock-playwright.db` 与 `.lock` 留在 Git 忽略目录；对应服务已按确认 PID 停止。隐藏重启脚本的重定向日志中文显示乱码，但脚本内备份路径、两次独立哈希校验、端口监听和健康检查均成功，源码文件未发生编码错误。
- 是否真实调用 LLM（provider/model）：本轮没有调用真实模型，浏览器验收使用 `mock`；正式 `8765` 保留已有硅基流动配置，但本轮未用正式账号发起问题。
- 是否修改源 `.maic.zip`：否。
- Git 提交：尚未暂存或提交；没有使用 `git add .`。
- 剩余边界：服务端以该用户、该 Quiz 单元是否存在权威测验结果作为解锁依据。未来若增加重做测验，需要显式建模“本次 attempt 已提交”，不能只删除客户端状态。若后续实现可视化溯源/自检卡片，应由服务端按问题意图返回结构化显示策略，视图只按策略渲染，不能让卡片默认常驻。
