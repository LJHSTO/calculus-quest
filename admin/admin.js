...
let cachedProactiveDecisionRows = [];
let cachedProactiveStudyData = {
  rows: [],
  total: 0,
  limit: 500,
    offset: 0,
  ...
  runtime: null,
  runtimeEvents: [],
  integrity: { advisoryOnly: true, summary: {}, signals: [] },
  participantFlow: [],
  participantAnalysis: [],
  coursewareChallenges: [],
  analysisSummary: {},
  dataQuality: { status: "pending", signals: [] },
  outcomes: {
    rows: [],
    total: 0,
    limit: 500,
    ...
    sessionSummary: [],
    instrumentVersion: "",
    retentionDelayHours: 0,
    retentionCloseHours: 0,
    retentionReminderHours: []
  }
};
let proactiveConditionFilter = sessionStorage.getItem("cq_proactive_condition") || "";
let proactiveDecisionFilter = sessionStorage.getItem("cq_proactive_decision") || "";
let selectedProactiveDecisionId = sessionStorage.getItem("cq_proactive_selected_decision") || "";
let proactiveAuditAllLoaded = false;
...
function hasAdminAccess() {
  return Boolean(localAdminAuth || adminToken);
}
...
async function checkAuth() {
  const result = await testAdminAuth();
  if (result.ok && result.authenticated) {
    localAdminAuth = result.localBypass;
    if (localAdminAuth) {
      adminToken = "";
...
function showApp() {
  ...
  document.getElementById("logout-btn")?.classList.toggle("hidden", localAdminAuth);
...
async function testAdminAuth() {
  try {
    const r = await fetch(`${API_BASE}/api/admin/auth/status`, {
...
function adminConnectionError(result = {}) {
  ...
  return `无法连接管理接口 ${API_BASE}/api/admin/auth/status${detail}`;
}
...
document.getElementById("login-btn").addEventListener("click", async () => {
  ...
  if (result.ok) {
    ...
    adminToken = "";
    document.getElementById("login-error").classList.remove("hidden");
    document.getElementById("login-error").textContent = "管理员密码无效，请重试。";
  }
});
...
function proactiveStudyParams({ limit = 500, offset = 0 } = {}) {
  const params = new URLSearchParams({
    limit: String(limit),
    offset: String(offset)
  });
  if (proactiveConditionFilter) params.set("condition", proactiveConditionFilter);
  if (proactiveDecisionFilter) params.set("decision", proactiveDecisionFilter);
  return params.toString();
}

async function fetchProactiveStudy(signal, options = {}) {
  const response = await fetch(
    `${API_BASE}/api/admin/research/proactive-decisions?${proactiveStudyParams(options)}`,
    {
      headers: adminRequestHeaders(),
      signal
    }
  ...
  return {
    rows: safeRows(payload.data?.rows),
    total: Number(payload.data?.total || 0),
    limit: Number(payload.data?.limit || options.limit || 500),
    offset: Number(payload.data?.offset || options.offset || 0),
    assignments: safeRows(payload.data?.assignments),
    assignmentRoster: {
      rows: safeRows(payload.data?.assignmentRoster?.rows),
      total: Number(payload.data?.assignmentRoster?.total || 0),
      limit: Number(payload.data?.assignmentRoster?.limit || options.limit || 500),
      offset: Number(payload.data?.assignmentRoster?.offset || options.offset || 0)
    },
    participation: safeRows(payload.data?.participation),
    summary: payload.data?.summary || {},
    policy: payload.data?.policy || {},
    protocol: payload.data?.protocol || null,
    runtime: payload.data?.runtime || null,
    runtimeEvents: safeRows(payload.data?.runtimeEvents),
    integrity: payload.data?.integrity || {
      advisoryOnly: true,
      summary: {},
      signals: []
    },
    participantFlow: safeRows(payload.data?.participantFlow),
    participantAnalysis: safeRows(payload.data?.participantAnalysis),
    coursewareChallenges: safeRows(payload.data?.coursewareChallenges),
    analysisSummary: payload.data?.analysisSummary || {},
    dataQuality: payload.data?.dataQuality || { status: "pending", signals: [] },
    outcomes: {
      rows: safeRows(payload.data?.outcomes?.rows),
      total: Number(payload.data?.outcomes?.total || 0),
      limit: Number(payload.data?.outcomes?.limit || options.limit || 500),
      offset: Number(payload.data?.outcomes?.offset || options.offset || 0),
      summary: safeRows(payload.data?.outcomes?.summary),
      sessions: safeRows(payload.data?.outcomes?.sessions),
      sessionTotal: Number(payload.data?.outcomes?.sessionTotal || 0),
      sessionLimit: Number(
        payload.data?.outcomes?.sessionLimit || options.limit || 500
      ),
      sessionOffset: Number(
        payload.data?.outcomes?.sessionOffset || options.offset || 0
      ),
      sessionSummary: safeRows(payload.data?.outcomes?.sessionSummary),
      instrumentVersion: payload.data?.outcomes?.instrumentVersion || "",
      retentionDelayHours: Number(
        payload.data?.outcomes?.retentionDelayHours || 0
      ),
      retentionCloseHours: Number(
        payload.data?.outcomes?.retentionCloseHours || 0
      ),
      retentionReminderHours: safeRows(
        payload.data?.outcomes?.retentionReminderHours
...
async function loadAll(signal) {
  ...
  try {
    ...
    const [overview, daily, userProg, chapter, questions, phase, qType, scoreDist, hourly, shortAnswers, feedbackDashboard, interactions, interactionDashboard, agenticTrace, proactiveStudyData] = await Promise.all([
    ...
    cachedProactiveStudyData = proactiveStudyData;
    cachedProactiveDecisionRows = safeRows(proactiveStudyData.rows);
    ...
    try { renderProactiveStudy(proactiveStudyData); } catch (e) { console.warn("Proactive study:", e); }
...
function interactionTypeName(type) {
  return ({
    ...
    knowledge_proactive_suggestion_snoozed: "稍后处理知点主动建议",
    knowledge_proactive_alternative_offered: "知点提供替代表征",
    knowledge_proactive_alternative_failed: "替代表征请求未完成",
    knowledge_proactive_preference_changed: "调整主动提示频率",
    knowledge_proactive_scope_preference_changed: "调整知识点主动提醒",
    knowledge_proactive_consent_notice_opened: "查看主动学习研究说明",
    knowledge_proactive_withdrawal_confirm_opened: "打开退出研究确认",
    knowledge_proactive_withdrawal_confirm_cancelled: "取消退出研究",
    knowledge_proactive_participation_enrolled: "加入本章主动学习研究",
    knowledge_proactive_participation_withdrawn: "退出本章主动学习研究",
    proactive_participation_enrolled: "服务端记录研究加入",
    proactive_participation_withdrawn: "服务端记录研究退出",
    knowledge_proactive_outcomes_opened: "打开独立学习测量",
    knowledge_proactive_outcome_started: "开始独立学习测量",
    knowledge_proactive_outcome_submitted: "提交独立学习测量",
    knowledge_proactive_outcome_failed: "独立学习测量提交失败",
    ...
    proactive_student_preference_changed: "服务端记录主动提示偏好",
    proactive_scope_preference_changed: "服务端记录知识点提醒偏好",
    proactive_alternative_requested: "服务端记录替代帮助请求",
    proactive_outcome_submitted: "服务端记录独立学习结果",
    proactive_verification_submitted: "服务端记录支架后检查",
    proactive_verification_declined: "服务端记录跳过支架后检查",
    proactive_student_response_submitted: "服务端记录支架回复",
...
function actionCategoryForType(type = "") {
  ...
  if (
    type.startsWith("knowledge_proactive_")
    || type.startsWith("knowledge_quiz_review_")
...
function proactiveSuggestionKindName(kind = "") {
  return ({
    ...
    quiz_review: "错题复盘",
    quiet_dwell: "停留后的澄清",
    formative_outcome: "形成性结果",
    independent_attempt_outcome: "独立尝试结果"
  })[kind] || "学习时机";
}
...
function proactiveActionName(action = "") {
  return ({
    ...
    elicit_self_explanation: "引导学生自我解释",
    concept_hint: "提供概念关系线索",
    decompose_subgoal: "拆分当前子目标",
    next_step_hint: "提示下一步",
    partial_worked_example: "查看部分解题示例",
    switch_representation: "切换学习表征",
    stay_silent: "保持安静"
  })[action] || "下一步支持";
}
...
function humanInteractionSummary(row) {
  ...
  if (type === "knowledge_proactive_suggestion_snoozed") {
    return `学生将「${proactiveActionName(data.action)}」设为稍后处理，本知识点进入冷却。`;
  }
  if (type === "knowledge_proactive_alternative_offered") {
    return `学生请求换一种帮助后，知点提供「${proactiveActionName(data.action)}」。`;
  }
  if (type === "knowledge_proactive_alternative_failed") {
    return `学生请求换一种帮助，但本次未能生成合法替代方案；原因：${data.reason || "未知"}。`;
  }
  if (type === "knowledge_proactive_preference_changed") {
    return `学生将主动提示频率从「${data.previousPreference || "未记录"}」改为「${data.preference || "未记录"}」。`;
  }
  if (type === "knowledge_proactive_scope_preference_changed") {
    return `学生将当前知识点主动提醒改为「${data.preference === "off" ? "关闭" : "恢复"}」。`;
  }
  if (type === "knowledge_proactive_consent_notice_opened") {
    return `学生打开本章研究说明，版本为 ${data.consentVersion || "未记录"}。`;
  }
  if (type === "knowledge_proactive_withdrawal_confirm_opened") {
    return "学生打开退出研究确认，但尚未提交退出。";
  }
  if (type === "knowledge_proactive_withdrawal_confirm_cancelled") {
    return "学生取消退出研究并继续保留参与状态。";
  }
  if (type === "knowledge_proactive_participation_enrolled") {
    return `学生确认参加本章主动学习研究，记录版本为 ${data.consentVersion || "未记录"}。`;
  }
  if (type === "knowledge_proactive_participation_withdrawn") {
    return "学生退出本章主动学习研究；课程与普通知点提问仍可继续使用。";
  }
  if (type === "proactive_participation_enrolled") {
    return `服务端已记录研究加入，依据为 ${data.enrollmentBasis || "未记录"}，版本为 ${data.consentVersion || "未记录"}。`;
  }
  if (type === "proactive_participation_withdrawn") {
    return `服务端已记录研究退出，并收束 ${Number(data.closedInterventions || 0)} 个未决提示、${Number(data.closedLifecycleEpisodes || 0)} 个支架回合和 ${Number(data.withdrawnOutcomeSessions || 0)} 个测量会话。`;
  }
  if (type === "knowledge_proactive_outcomes_opened") {
    return "学生打开本章独立学习测量；主动提示在测量期间暂停。";
  }
  if (type === "knowledge_proactive_outcome_started") {
    return `学生开始${proactiveOutcomeStageLabel(data.stageId)}，共 ${Number(data.itemCount || 0)} 题。`;
  }
  if (type === "knowledge_proactive_outcome_submitted") {
    return `学生提交${proactiveOutcomeStageLabel(data.stageId)}；学生端只确认记录成功，不显示即时得分。`;
  }
  if (type === "knowledge_proactive_outcome_failed") {
    return `${proactiveOutcomeStageLabel(data.stageId)}提交未完成；原因：${data.reason || "请求失败"}。`;
  }
  if (type === "proactive_student_preference_changed") {
    return `服务端确认主动提示偏好改为「${data.preference || "未记录"}」，并关闭 ${Number(data.closedInterventions || 0)} 条未完成建议。`;
  }
  if (type === "proactive_scope_preference_changed") {
    return `服务端确认知识点「${data.scopeKey || payload.unitId || ""}」提醒已${data.preference === "off" ? "关闭" : "恢复"}。`;
  }
  if (type === "proactive_alternative_requested") {
    return `服务端核验原建议后，记录学生主动请求替代帮助；原支架为 ${data.sourceSupportLevel || "未记录"}。`;
  }
  if (type === "proactive_outcome_submitted") {
    return `服务端记录${proactiveOutcomeStageLabel(data.stageId)}，共 ${Number(data.itemCount || 0)} 题；结果仅进入研究后台。`;
  }
  if (type === "proactive_verification_submitted") {
    return `服务端记录支架后检查「${data.checkId || "未标记"}」${data.instrumentVersion ? `（${data.instrumentVersion}）` : ""}，用于决定渐退或升级支架，不等同于独立结果测量。`;
  }
  if (type === "proactive_verification_declined") {
    return "学生跳过本轮支架后检查，服务端结束当前支架回合且不自动升级。";
  }
  if (type === "proactive_student_response_submitted") {
    return "服务端确认学生已提交本轮支架回复，下一步等待独立尝试证据。";
  }
...
// ---- Proactive study ----
const proactiveCandidateLabels = {
  repeated_parameter: "连续调参",
  quiet_dwell: "安静停留",
  quiz_review: "错题复盘",
  formative_outcome: "形成性测验结果",
  independent_attempt_outcome: "支架后独立尝试"
};

const proactiveActionLabels = {
  stay_silent: "保持安静",
  observe_change: "引导观察变化",
  review_mistake: "复盘错因",
  ask_clarification: "澄清卡点",
  self_explain: "自我解释",
  elicit_self_explanation: "引导自我解释",
  concept_hint: "概念关系线索",
  decompose_subgoal: "拆分子目标",
  next_step_hint: "提示下一步",
  partial_worked_example: "部分解题示例",
  switch_representation: "切换学习表征"
};

const proactiveResolutionLabels = {
  accepted: "已接受",
  dismissed: "已关闭",
  ignored: "已忽略",
  snoozed: "稍后处理",
  alternative_requested: "已请求换一种帮助",
  scope_muted: "已关闭知识点提醒",
  preference_off: "已关闭全局主动提示",
  learning_reset: "学习代次已重置",
  experiment_paused: "实验暂停时收束",
  expired: "已过期",
  shown: "已展示",
  offered: "待展示"
};

const proactiveEvidenceSourceLabels = {
  policy_owned_probe: "平台支架后检查题",
  courseware_semantic_event: "课件语义结果事件",
  interaction: "学习交互事件",
  quiz_result: "平台测验结果",
  proactive_intervention: "主动干预记录"
};

const proactiveEvidenceEventLabels = {
  proactive_verification_submitted: "支架后检查已提交",
  proactive_verification_declined: "学生跳过支架后检查",
  courseware_challenge_result: "课件挑战结果",
  courseware_formative_check_submitted: "课件形成性检查结果",
  parameter_commit: "参数提交",
  courseware_interaction_complete: "课件互动完成",
  courseware_observable_evidence_captured: "课件可观察证据"
};

function proactiveEvidenceSourceLabel(source = "", eventType = "") {
  const sourceLabel = proactiveEvidenceSourceLabels[source] || source || "未标记来源";
  const eventLabel = proactiveEvidenceEventLabels[eventType] || eventType || "";
  return eventLabel ? `${sourceLabel} · ${eventLabel}` : sourceLabel;
}

function proactiveConditionLabel(value = "") {
  return ({
    treatment: "主动提示组",
    control: "对照组",
    treatment_shallow: "浅层主动组",
    treatment_full: "完整主动组",
    minimal: "最小主动组",
    adaptive: "自适应主动组"
  })[value] || value || "未分配";
}

function proactiveStratumLabel(value = "") {
  return value === "pre-high"
    ? "前测较高"
    : value === "pre-low"
      ? "前测较低"
      : "未分层";
...
function proactiveDecisionLabel(value = "") {
  return value === "intervene" ? "应介入" : "保持安静";
}

function proactiveDeliveryLabel(value = "") {
  return value === "intervene" ? "已签发展示" : "实际静默";
}

function proactiveScore(value) {
  const number = Number(value);
  return Number.isFinite(number) ? number.toFixed(2) : "-";
}

function proactiveReasonText(reasons) {
  const labels = {
    repeated_parameter_verified: "连续调参已核验",
    natural_parameter_boundary: "处于自然操作边界",
    confirmed_quiz_error: "存在已确认错题",
    confirmed_formative_error: "形成性测验确认错误",
    quiz_submission_boundary: "测验已提交",
    quiet_dwell_with_confirmed_gap: "停留且存在已确认薄弱点",
    quiet_dwell_time_only: "只有停留证据",
    quiet_dwell_waiting_for_boundary: "已登记 HelpNeed，等待自然边界",
    quiet_dwell_boundary_not_verified: "自然边界尚未核验",
    quiet_dwell_pending_absent: "没有待评估停留候选",
    natural_task_boundary: "处于自然任务边界",
    independent_task_boundary: "处于支架后的独立任务边界",
    continued_friction_after_self_explanation: "L1 后独立尝试仍失败，允许升级至 L2",
    independent_attempt_success: "独立尝试成功，渐退并退出",
    accepted_response_pending: "已接受支架，等待学生实际回复",
    independent_attempt_required: "等待支架后检查或课件语义结果",
    independent_attempt_not_expected: "当前生命周期不接受独立结果",
    independent_attempt_not_verified: "没有服务端可核验的独立任务证据",
    verification_declined_without_escalation: "学生跳过验证，本轮结束且不升级",
    verification_check_already_submitted: "同一道验证题已提交，阻止重放",
    student_response_submitted: "学生已完成本轮支架回复",
    support_declined: "学生已拒绝，本轮闭环终止",
    student_snoozed_support: "学生选择稍后处理",
    student_requested_alternative: "学生主动请求换一种帮助",
    student_choice_boundary: "学生主动选择形成合法介入边界",
    lifecycle_closed: "本轮支架闭环已经结束",
    maximum_support_reached: "已到本轮最高支架强度，停止升级",
    policy_thresholds_met: "达到政策门槛",
    shadow_mode_suppressed: "Shadow 不展示",
    control_condition_suppressed: "Control 不展示",
    protected_assessment_stage: "受保护测量阶段",
    experiment_assignment_pending: "前测完成前不分组",
    intervention_budget_exhausted: "主动预算已用尽",
    policy_cooldown_active: "学生冷却期",
    open_intervention_exists: "同一知识点已有待处理建议",
    repeated_rejection_guard: "连续拒绝保护",
    evidence_families_insufficient: "证据族不足",
    help_need_below_threshold: "HelpNeed 未达门槛",
    interruptibility_below_threshold: "当前不宜打断",
    uncertainty_above_threshold: "不确定性过高",
    repeated_parameter_not_verified: "连续调参未核验",
    progress_already_resumed: "学习进展已恢复",
    confirmed_error_absent: "没有已确认错题",
    grading_incomplete: "评分尚未完成",
    formative_outcome_scope_invalid: "形成性结果不在合法测验范围",
    formative_knowledge_point_absent: "形成性结果缺少知识点映射",
    independent_attempt_scope_invalid: "独立结果缺少知识点映射",
    candidate_kind_unsupported: "当前 Adapter 不支持该候选",
    adapter_unit_unsupported: "当前单元未纳入 Adapter",
    chapter_not_managed: "当前章节不属于本试点",
    policy_mode_off: "主动政策已关闭",
    experiment_runtime_paused: "实验运行时已暂停",
    student_preference_reduced: "学生选择少提醒，仅保留强证据介入",
    student_preference_off: "学生已关闭主动提示",
    fade_threshold_raised: "渐退后提高弱证据介入门槛",
    fade_evidence_threshold_raised: "渐退后要求更多独立证据",
    fade_evidence_requirement_met: "渐退后的更高证据要求已满足",
    experiment_arm_display_suppressed: "当前实验臂不展示主动介入",
    arm_support_ceiling: "当前实验臂已达到支架上限",
    arm_representation_switch_disabled: "当前实验臂不允许换表征",
    assignment_salt_required: "分组盐未配置，Active 失败关闭",
    participation_config_required: "研究参与配置未完成，Active 失败关闭",
    participant_not_enrolled: "学生尚未参加本章研究",
    participant_withdrawn: "学生已退出本章研究",
    no_legal_action: "没有满足教学契约的合法动作",
    renderer_action_mismatch: "呈现动作不匹配",
    support_escalation_not_delivered: "升级支架未实际交付，本轮已安全结束"
  };
  return safeRows(reasons).map((reason) => labels[reason] || reason).join("、") || "无";
}

function proactiveAuditTime(value = "") {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return String(value || "未记录");
  return date.toLocaleString("zh-CN", {
    year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit"
  });
}

function proactiveAssignmentStrategyLabel(value = "") {
  return value === "stratified-balanced"
    ? "层内小样本平衡"
    : "层内加盐哈希";
}

function proactiveRuntimeLabel(status = "") {
  return status === "paused" ? "已暂停" : "运行中";
}

function proactiveProtocolFact(label, value, title = "") {
  return `
    <div class="proactive-protocol-fact"${title ? ` title="${esc(title)}"` : ""}>
      <span>${esc(label)}</span>
...
function renderProactiveGovernance(data = {}) {
  const policy = data.policy || {};
  const protocol = data.protocol || null;
  const definition = protocol?.definition || {};
  const participation = safeRows(data.participation);
  const enrolledParticipants = participation
    .filter((row) => row.status === "enrolled")
    .reduce((sum, row) => sum + Number(row.participants || 0), 0);
  const withdrawnParticipants = participation
    .filter((row) => row.status === "withdrawn")
    .reduce((sum, row) => sum + Number(row.participants || 0), 0);
  const runtime = data.runtime || null;
  const runtimeStatus = runtime?.status === "paused" ? "paused" : "active";
  const statusNode = document.getElementById("proactive-runtime-status");
  if (statusNode) {
    statusNode.className = `proactive-runtime-badge is-${runtimeStatus}`;
    statusNode.textContent = runtime
      ? proactiveRuntimeLabel(runtimeStatus)
      : "未配置";
  }
  const runtimeToggle = document.getElementById("proactive-runtime-toggle");
  if (runtimeToggle) {
    runtimeToggle.disabled = !runtime;
    runtimeToggle.dataset.nextStatus = runtimeStatus === "paused" ? "active" : "paused";
    runtimeToggle.textContent = runtimeStatus === "paused" ? "恢复实验" : "暂停实验";
    runtimeToggle.classList.toggle("btn-primary", runtimeStatus === "paused");
    runtimeToggle.classList.toggle("btn-outline", runtimeStatus !== "paused");
  }
  const reasonInput = document.getElementById("proactive-pause-reason");
  if (reasonInput && document.activeElement !== reasonInput) {
    reasonInput.value = runtimeStatus === "paused" ? runtime?.reason || "" : "";
  }
  const facts = document.getElementById("proactive-protocol-facts");
  if (facts) {
    const fingerprint = protocol?.fingerprint || "";
    const modelRuntime = definition.modelRuntime || {};
    const sourceDigests = definition.sourceDigests || {};
    facts.innerHTML = [
      proactiveProtocolFact(
        "实验 / 章节",
        `${protocol?.experimentId || policy.experimentId || "未配置"} · ${protocol?.chapterId || policy.chapterId || "未配置"}`
      ),
      proactiveProtocolFact(
        "协议指纹",
        fingerprint ? `${fingerprint.slice(0, 16)}…` : "未冻结",
        fingerprint
      ),
      proactiveProtocolFact(
        "冻结时间",
        protocol?.frozenAt ? proactiveAuditTime(protocol.frozenAt) : "未冻结"
      ),
      proactiveProtocolFact(
        "课程版本",
        definition.courseRouteVersion || "未记录",
        definition.courseRouteDigest || ""
      ),
      proactiveProtocolFact(
        "分配策略",
        `${proactiveAssignmentStrategyLabel(policy.assignmentStrategy)} · ${policy.assignmentVersion || "未版本化"}`
      ),
      proactiveProtocolFact(
        "参与模式",
        policy.participationMode === "explicit-consent"
          ? `显式同意 · ${policy.consentVersion || "未版本化"}`
          : `工程试点 · ${policy.consentVersion || "未版本化"}`
      ),
      proactiveProtocolFact(
        "参与状态",
        `在研 ${enrolledParticipants} · 已退出 ${withdrawnParticipants}`
      ),
      proactiveProtocolFact(
        "政策版本",
        `${policy.policyVersion || "未记录"} · ${policy.adapterVersion || "未记录"}`
      ),
      proactiveProtocolFact(
        "模型配置",
        modelRuntime.liveConfigured
          ? `${modelRuntime.provider || "模型"} · ${modelRuntime.model || "未指定模型"}`
          : `${modelRuntime.provider || "mock"} · 未启用实时生成`
      ),
      proactiveProtocolFact(
        "冻结源码",
        `${Object.keys(sourceDigests).length} 个摘要 · ${definition.outcomeInstrumentVersion || "测量版本未记录"}`
      )
    ].join("");
  }

  const integrity = data.integrity || {};
  const integrityStatus = document.getElementById("proactive-integrity-status");
  if (integrityStatus) {
    const summary = integrity.summary || {};
    integrityStatus.textContent = Number(summary.critical || 0) > 0
      ? `${Number(summary.critical || 0)} 个需立即复核`
      : Number(summary.warning || 0) > 0
        ? `${Number(summary.warning || 0)} 个观察信号`
        : "当前无违规信号";
  }
  const integrityList = document.getElementById("proactive-integrity-signals");
  if (integrityList) {
    const signals = safeRows(integrity.signals);
    integrityList.innerHTML = signals.length
      ? signals.map((signal) => `
          <div class="proactive-integrity-signal is-${esc(signal.status || "ok")}">
            <div>
              <strong>${esc(signal.label || signal.id || "未命名信号")}</strong>
              <p>${esc(signal.detail || "")}</p>
            </div>
            <b>${Number(signal.count || 0)}</b>
          </div>
        `).join("")
      : '<p class="inline-empty-state">尚未生成停止规则诊断。</p>';
  }

  const flowBody = document.querySelector("#table-proactive-participant-flow tbody");
  if (flowBody) {
    const participantFlow = safeRows(data.participantFlow);
    flowBody.innerHTML = participantFlow.length
      ? participantFlow.map((row) => {
          const withdrawn = Number(row.nearWithdrawn || 0)
            + Number(row.representationWithdrawn || 0)
            + Number(row.retentionWithdrawn || 0);
          return `<tr>
            <td><span class="badge ${proactiveConditionBadgeClass(policy, row.condition)}">${esc(proactiveConditionLabel(row.condition))}</span></td>
            <td>${Number(row.assigned || 0)}</td>
            <td>${Number(row.evidenceParticipants || 0)}</td>
            <td>${Number(row.exposedParticipants || 0)}</td>
            <td>${Number(row.posttestCompleted || 0)}</td>
            <td>${Number(row.nearSubmitted || 0)} / ${Number(row.nearStarted || 0)}</td>
            <td>${Number(row.representationSubmitted || 0)} / ${Number(row.representationStarted || 0)}</td>
            <td>${Number(row.retentionSubmitted || 0)} / ${Number(row.retentionStarted || 0)}</td>
            <td>${Number(row.studyWithdrawn || 0)}</td>
            <td>${withdrawn}</td>
          </tr>`;
        }).join("")
      : "<tr><td colspan='10'>尚无已分组参与者。</td></tr>";
    syncTableDensity(flowBody.closest("table"));
  }

  const assignmentRoster = data.assignmentRoster || {};
  const assignmentRows = safeRows(assignmentRoster.rows);
  const assignmentRosterStatus = document.getElementById("proactive-assignment-roster-status");
  if (assignmentRosterStatus) {
    assignmentRosterStatus.textContent = Number(assignmentRoster.total || 0) > assignmentRows.length
      ? "最近 " + assignmentRows.length + " / " + Number(assignmentRoster.total || 0) + " 人"
      : assignmentRows.length + " 人；分组固定且保留用于 ITT";
  }
  const assignmentRosterBody = document.querySelector("#table-proactive-assignment-roster tbody");
  if (assignmentRosterBody) {
    assignmentRosterBody.innerHTML = assignmentRows.length
      ? assignmentRows.map((row) => (
          "<tr>"
          + "<td><strong>" + esc(row.nickname || "未命名学生") + "</strong><br><span class=\"muted\">" + esc(row.user_id || "") + "</span></td>"
          + "<td><span class=\"badge " + proactiveConditionBadgeClass(policy, row.condition) + "\">" + esc(proactiveConditionLabel(row.condition)) + "</span></td>"
          + "<td>" + esc(proactiveStratumLabel(row.stratum)) + "</td>"
          + "<td>" + esc(row.assignment_version || "未版本化") + "</td>"
          + "<td style=\"white-space:nowrap;\">" + esc(proactiveAuditTime(row.assigned_at || "")) + "</td>"
          + "<td>" + esc(row.participation_status === "withdrawn" ? "已退出（保留分组）" : row.participation_status === "enrolled" ? "在研" : "未记录") + "</td>"
          + "<td>第 " + Math.max(1, Number(row.learning_generation || 1)) + " 次</td>"
          + "</tr>"
        )).join("")
      : "<tr><td colspan='7'>尚无已完成前测并固定分组的学生。</td></tr>";
    syncTableDensity(assignmentRosterBody.closest("table"));
  }

  const runtimeBody = document.querySelector("#table-proactive-runtime-events tbody");
  if (runtimeBody) {
    const events = safeRows(data.runtimeEvents);
    runtimeBody.innerHTML = events.length
      ? events.map((row) => `<tr>
          <td style="white-space:nowrap;">${esc(proactiveAuditTime(row.created_at || ""))}</td>
          <td><span class="badge ${row.status === "paused" ? "badge-amber" : "badge-green"}">${esc(proactiveRuntimeLabel(row.status))}</span></td>
          <td>${esc(row.updated_by || "admin")}</td>
          <td>${esc(row.reason || (row.status === "active" ? "恢复既有冻结协议" : ""))}</td>
        </tr>`).join("")
      : "<tr><td colspan='4'>尚无人工暂停或恢复记录。</td></tr>";
    syncTableDensity(runtimeBody.closest("table"));
  }
}

function proactiveAuditJson(value, fallback = {}) {
  const parsed = parseJsonMaybe(value);
  if (parsed && typeof parsed === "object") {
    try { return JSON.stringify(parsed, null, 2); } catch {}
  }
  try { return JSON.stringify(fallback, null, 2); } catch { return "{}"; }
}

function proactiveAuditDisclosure(label, value, fallback = {}, hint = "查看原始 JSON") {
  return [
    '<details class="proactive-audit-disclosure">',
    '<summary><span>', esc(label), '</span><small>', esc(hint), '</small></summary>',
    '<pre class="proactive-audit-code">', esc(proactiveAuditJson(value, fallback)), '</pre>',
    '</details>'
  ].join("");
}

function proactiveAuditStatus(row = {}) {
  const resolution = row.resolution || row.intervention_status || "";
  return proactiveResolutionLabels[resolution]
    || (row.delivery_decision === "intervene" ? "待展示" : "未展示");
}

function proactiveAuditReasonItems(row = {}) {
  return safeRows(row.reason_codes).map((reason) => ({
    code: reason,
    label: proactiveReasonText([reason])
  }));
}

function proactiveAuditFact(label, value) {
  return '<div class="proactive-audit-detail-fact"><span>'
    + esc(label)
    + '</span><strong>'
    + esc(value || "未记录")
    + '</strong></div>';
...
function proactiveLifecyclePhaseLabel(value = "") {
  return ({
    idle: "尚未开启",
    accepted_pending_response: "已接受，等待学生回复",
    awaiting_independent_attempt: "等待支架后检查或课件语义结果",
    deferred: "学生选择稍后，本轮暂缓",
    declined: "学生拒绝，本轮已退出",
    exited: "独立成功，已渐退退出",
    closed: "升级未交付，本轮已安全结束",
    maximum_support_reached: "达到本轮最高支架，已停止升级"
  })[value] || value || "尚未开启";
}

function proactiveOutcomeLabel(value = "") {
  return ({
    student_response_submitted: "学生已提交自我解释",
    continued_friction: "独立尝试失败，仍有卡点",
    continued_friction_after_concept_hint: "L2 后仍失败，停止继续升级",
    continued_friction_after_subgoal_decomposition: "L3 后仍有卡点",
    continued_friction_after_next_step_hint: "L4 后仍有卡点",
    continued_friction_after_partial_worked_example: "L5 后仍有卡点，停止升级",
    independent_attempt_success: "独立尝试成功",
    support_escalation_not_delivered: "升级支架未交付，本轮安全结束",
    verification_declined: "学生跳过验证，本轮结束",
    snoozed: "学生选择稍后处理",
    alternative_requested: "学生请求换一种帮助",
    dismissed: "学生关闭建议",
    ignored: "建议被忽略"
  })[value] || value || "";
}

function proactiveFollowupSummary(row = {}) {
  const outcome = row.followup_outcome || {};
  const response = outcome.studentResponse || {};
  const attempt = outcome.independentAttempt || {};
  const parts = [];
  if (response.submitted) {
    const length = Number(response.responseLength || 0);
    parts.push(`学生已回复${length ? ` ${length} 字` : ""}`);
  }
  if (attempt.outcome) {
    parts.push(proactiveOutcomeLabel(attempt.outcome));
  }
  if (attempt.source || attempt.eventType) {
    parts.push(proactiveEvidenceSourceLabel(attempt.source, attempt.eventType));
  }
  return parts.join(" · ");
}

function proactiveEvidenceItems(row = {}) {
  const refs = safeRows(row.evidence_refs);
  const attempt = row.followup_outcome?.independentAttempt || {};
  const items = refs.map((ref) => ({
    id: String(ref.id || ""),
    label: proactiveEvidenceSourceLabel(ref.sourceType, ref.eventType)
  }));
  if (attempt.source || attempt.eventType) {
    items.push({
      id: String(attempt.checkId || ""),
      label: proactiveEvidenceSourceLabel(attempt.source, attempt.eventType)
    });
  }
  const seen = new Set();
...
function proactiveScopeKey(row = {}) {
  return String(row.scope_key || row.unit_id || "");
}

function proactiveLearningGeneration(row = {}) {
  return Math.max(1, Number(row.learning_generation || 1));
}

function proactiveStateSummary(snapshot = {}) {
  const lifecycle = snapshot.lifecycle
    && typeof snapshot.lifecycle === "object"
    ? snapshot.lifecycle
    : {};
  return {
    phase: proactiveLifecyclePhaseLabel(lifecycle.phase),
    level: snapshot.currentLevel || lifecycle.supportLevel || "L0",
    fadeLevel: Math.max(0, Number(snapshot.fadeLevel || 0)),
    preference: ({
      standard: "标准",
      reduced: "少提醒",
      off: "关闭"
    })[snapshot.studentPreference] || snapshot.studentPreference || "标准",
    reason: proactiveReasonText([
      lifecycle.reasonCode || snapshot.lastReasonCode || ""
    ].filter(Boolean))
  };
}

function proactiveStateTransitionHtml(row = {}) {
  const before = proactiveStateSummary(
    row.evidence_snapshot?.policyStateBefore || {}
  );
  const transition = row.evidence_snapshot?.stateTransition || {};
  const hasTransition = Boolean(
    transition
    && typeof transition === "object"
    && Object.keys(transition).length
  );
  const transitionState = hasTransition
    ? proactiveStateSummary({
    ...
    : {
        phase: "状态未改变",
        level: before.level,
        fadeLevel: before.fadeLevel,
        preference: before.preference,
        reason: "本次决策只记录判断，没有更新支架生命周期"
      };
  const after = proactiveStateSummary(
    row.evidence_snapshot?.policyStateAfter || {}
  );
  const stateBlock = (label, state, tone = "") => [
    '<div class="proactive-audit-transition-state ', tone, '">',
    '<span>', esc(label), '</span>',
    '<strong>', esc(state.phase), '</strong>',
    '<small>强度 ', esc(state.level), ' · 渐退 ', esc(state.fadeLevel), ' 次 · ', esc(state.preference), '</small>',
    state.reason && state.reason !== "无"
      ? '<small>' + esc(state.reason) + '</small>'
      : "",
    '</div>'
  ].join("");
  return [
    '<div class="proactive-audit-transition">',
    stateBlock("决策前", before),
    '<div class="proactive-audit-transition-arrow" aria-hidden="true">→</div>',
    stateBlock(
      "本次转移",
      transitionState,
      hasTransition ? "is-transition" : "is-unchanged"
    ),
    '<div class="proactive-audit-transition-arrow" aria-hidden="true">→</div>',
    stateBlock("决策后", after, "is-after"),
    '</div>'
  ].join("");
}

function proactiveAuditChain(rows = [], selected = {}) {
  const scopeKey = proactiveScopeKey(selected);
  const learningGeneration = proactiveLearningGeneration(selected);
  const chain = safeRows(rows)
    .filter((row) => (
      row.user_id === selected.user_id
      && row.experiment_id === selected.experiment_id
      && proactiveScopeKey(row) === scopeKey
      && proactiveLearningGeneration(row) === learningGeneration
    ))
    .sort((left, right) => (
      Date.parse(left.created_at || "") - Date.parse(right.created_at || "")
      || String(left.id || "").localeCompare(String(right.id || ""))
    ));
  const isCurrentGeneration = learningGeneration === Math.max(
    1,
    Number(selected.current_learning_generation || learningGeneration)
  );
  const currentLifecycle = selected.current_lifecycle
    && typeof selected.current_lifecycle === "object"
    ? selected.current_lifecycle
    : {};
  const lifecycle = isCurrentGeneration && Object.keys(currentLifecycle).length
    ? currentLifecycle
    : selected.evidence_snapshot?.policyStateAfter?.lifecycle || {};
  const currentPhase = proactiveLifecyclePhaseLabel(lifecycle.phase);
  const currentLevel = isCurrentGeneration
    ? selected.current_policy_level || lifecycle.supportLevel || "L0"
    : selected.evidence_snapshot?.policyStateAfter?.currentLevel
      || lifecycle.supportLevel
      || "L0";
  const fadeLevel = Math.max(0, Number(
    isCurrentGeneration
      ? selected.current_fade_level || 0
      : selected.evidence_snapshot?.policyStateAfter?.fadeLevel || 0
  ));
  const stateSummary = [
    `${isCurrentGeneration ? "当前状态" : "该次决策后状态"}：${currentPhase}`,
    `强度 ${currentLevel}`,
    fadeLevel ? `渐退 ${fadeLevel} 次` : "",
    `第 ${learningGeneration} 次学习`
  ].filter(Boolean).join(" · ");

  return [
    '<div class="proactive-audit-chain-state"><strong>', esc(stateSummary), '</strong>',
    isCurrentGeneration && selected.policy_state_updated_at
      ? '<span>状态更新于 ' + esc(proactiveAuditTime(selected.policy_state_updated_at)) + '</span>'
      : "",
    '</div>',
    '<ol class="proactive-audit-chain">',
    chain.map((row) => {
      const followup = proactiveFollowupSummary(row);
      const action = proactiveActionLabels[row.action] || row.action || "无动作";
      const candidate = proactiveCandidateLabels[row.candidate_kind]
        || row.candidate_kind
        || "未知候选";
      const classes = [
        "proactive-audit-chain-step",
        row.id === selected.id ? "is-selected" : "",
        row.support_level === "L1" ? "is-l1" : "",
        row.support_level === "L2" ? "is-l2" : "",
        row.support_level === "L3" ? "is-l3" : "",
        row.support_level === "L4" ? "is-l4" : "",
        row.support_level === "L5" ? "is-l5" : "",
        row.followup_outcome?.independentAttempt?.outcome === "independent_attempt_success"
          ? "is-success"
          : ""
      ].filter(Boolean).join(" ");
      return [
        '<li class="', classes, '">',
        '<span class="proactive-audit-chain-time">', esc(proactiveAuditTime(row.created_at)), '</span>',
        '<strong>', esc(candidate), ' → ', esc(action), ' · ', esc(row.support_level || "L0"), '</strong>',
        '<p>', esc(proactiveDecisionLabel(row.decision)), '，', esc(proactiveDeliveryLabel(row.delivery_decision)), '；', esc(proactiveAuditStatus(row)), '。</p>',
        followup ? '<p class="proactive-audit-chain-outcome">' + esc(followup) + '。</p>' : "",
        '<small>', esc(proactiveReasonText(row.reason_codes)), '</small>',
        '</li>'
      ].join("");
    }).join(""),
    '</ol>'
...
function renderProactiveAudit(data = {}) {
  const timeline = document.getElementById("proactive-audit-timeline");
  const detail = document.getElementById("proactive-audit-detail");
  const status = document.getElementById("proactive-audit-status");
  const count = document.getElementById("proactive-audit-timeline-count");
  if (!timeline || !detail) return;

  const rows = safeRows(data.rows);
  if (!rows.length) {
    if (status) status.textContent = "当前筛选条件下暂无决策";
    if (count) count.textContent = "0 条";
    timeline.innerHTML = '<p class="inline-empty-state proactive-audit-empty">当前筛选条件下暂无主动策略事件。</p>';
    detail.innerHTML = '<p class="inline-empty-state">没有可查看的决策详情。</p>';
    return;
  }

  const selected = rows.find((row) => row.id === selectedProactiveDecisionId) || rows[0];
  selectedProactiveDecisionId = selected.id || "";
  if (selectedProactiveDecisionId) {
    sessionStorage.setItem("cq_proactive_selected_decision", selectedProactiveDecisionId);
  }
  if (status) {
    const total = Number(data.total || rows.length);
    status.textContent = proactiveAuditAllLoaded
      ? "已载入全部 " + rows.length + " 条决策"
      : (total > rows.length
        ? "时间线显示最近 " + rows.length + " / " + total + " 条"
        : rows.length + " 条决策");
  }
  if (count) count.textContent = rows.length + " 条";

  timeline.innerHTML = rows.map((row) => {
    const isSelected = row.id === selectedProactiveDecisionId;
    const delivered = row.delivery_decision === "intervene";
    const classes = [
      "proactive-audit-event",
      row.decision === "intervene" ? "is-intervene" : "",
      delivered ? "is-delivered" : "",
      isSelected ? "is-selected" : ""
    ].filter(Boolean).join(" ");
    const title = proactiveDecisionLabel(row.decision) + " · " + proactiveDeliveryLabel(row.delivery_decision);
      const location = chapterName(row.chapter_id) + " · " + moduleName(row.unit_id, row.unit_id || "");
    return [
      '<button class="', classes, '" type="button" data-proactive-audit-id="', esc(row.id || ""), '" aria-pressed="', isSelected ? "true" : "false", '">',
      '<span class="proactive-audit-event-header"><span class="proactive-audit-event-title">', esc(title), '</span><span class="proactive-audit-event-time">', esc(proactiveAuditTime(row.created_at)), '</span></span>',
      '<span class="proactive-audit-event-meta">', esc(row.nickname || "匿名学生"), ' · 第 ', esc(proactiveLearningGeneration(row)), ' 次学习 · ', esc(location), '</span>',
      '<span class="proactive-audit-event-meta">', esc(proactiveCandidateLabels[row.candidate_kind] || row.candidate_kind || "未知候选"), ' · ', esc(proactiveActionLabels[row.action] || row.action || "无动作"), ' · ', esc(proactiveAuditStatus(row)), '</span>',
      '<span class="proactive-audit-event-reason">', esc(proactiveReasonText(row.reason_codes)), '</span>',
      '<span class="proactive-audit-event-facts"><span>H ', esc(proactiveScore(row.help_need_score)), '</span><span>I ', esc(proactiveScore(row.interruptibility_score)), '</span><span>U ', esc(proactiveScore(row.uncertainty_score)), '</span></span>',
      '</button>'
    ].join("");
  }).join("");

  const row = selected;
  const reasons = proactiveAuditReasonItems(row);
  const evidenceRefs = safeRows(row.evidence_refs);
  const evidenceItems = proactiveEvidenceItems(row);
  const interventionState = row.intervention_status
    || (row.delivery_decision === "intervene" ? "待展示" : "未创建");
  const outcome = row.followup_outcome
    && typeof row.followup_outcome === "object"
    && Object.keys(row.followup_outcome).length
    ? row.followup_outcome
    : {};
  const evidenceFamilies = safeRows(row.evidence_snapshot?.families || []);
  const currentLifecycle = row.current_lifecycle
    && typeof row.current_lifecycle === "object"
    ? row.current_lifecycle
    : {};
  const isCurrentGeneration = proactiveLearningGeneration(row) === Math.max(
    1,
    Number(row.current_learning_generation || proactiveLearningGeneration(row))
  );
  const displayedLifecycle = isCurrentGeneration && Object.keys(currentLifecycle).length
    ? currentLifecycle
    : row.evidence_snapshot?.policyStateAfter?.lifecycle || {};
  detail.innerHTML = [
    '<div class="proactive-audit-detail-header"><div><h4>',
    esc(proactiveDecisionLabel(row.decision) + " · " + (proactiveCandidateLabels[row.candidate_kind] || row.candidate_kind || "未知候选")),
    '</h4><p>', esc(row.id || ""), ' · ', esc(proactiveAuditTime(row.created_at)), '</p></div>',
    '<span class="badge ', row.delivery_decision === "intervene" ? "badge-green" : "badge-blue", '">', esc(proactiveDeliveryLabel(row.delivery_decision)), '</span></div>',
    '<div class="proactive-audit-detail-grid">',
    proactiveAuditFact("学生", row.nickname || row.user_id),
    proactiveAuditFact("学习代次", `第 ${proactiveLearningGeneration(row)} 次学习`),
    proactiveAuditFact("学习位置", chapterName(row.chapter_id) + " · " + moduleName(row.unit_id, row.unit_id || "")),
    proactiveAuditFact("策略范围", row.scope_key || row.unit_id),
    proactiveAuditFact("实验条件", proactiveConditionLabel(row.condition)),
    proactiveAuditFact("策略版本", row.policy_version),
    proactiveAuditFact("动作 / 支架", (proactiveActionLabels[row.action] || row.action || "无动作") + " · " + (row.support_level || "L0")),
    proactiveAuditFact("干预状态", interventionState),
    proactiveAuditFact(
      isCurrentGeneration ? "当前生命周期" : "该次决策后生命周期",
      proactiveLifecyclePhaseLabel(displayedLifecycle.phase)
    ),
    '</div>',
    '<section class="proactive-audit-detail-section"><h5>决策状态转移</h5>',
    proactiveStateTransitionHtml(row),
    '</section>',
    '<section class="proactive-audit-detail-section"><h5>知识点策略链</h5>',
    proactiveAuditChain(rows, row),
    '</section>',
    '<section class="proactive-audit-detail-section"><h5>评分与证据族</h5><div class="decision-facts"><span>HelpNeed ', esc(proactiveScore(row.help_need_score)), '</span><span>可打断性 ', esc(proactiveScore(row.interruptibility_score)), '</span><span>不确定性 ', esc(proactiveScore(row.uncertainty_score)), '</span>',
    evidenceFamilies.map((family) => '<span>' + esc(family) + '</span>').join(""),
    '</div></section>',
    '<section class="proactive-audit-detail-section"><h5>结构化理由</h5><div class="proactive-audit-reasons">',
    reasons.length
      ? reasons.map((item) => '<span class="proactive-audit-reason" title="' + esc(item.code) + '">' + esc(item.label) + '</span>').join("")
      : '<span class="muted">无</span>',
    '</div></section>',
    '<section class="proactive-audit-detail-section"><h5>证据来源</h5><div class="proactive-audit-reasons">',
    evidenceItems.length
      ? evidenceItems.map((item) => '<span class="proactive-audit-reason" title="' + esc(item.id) + '">' + esc(item.label) + '</span>').join("")
      : '<span class="muted">无</span>',
    '</div></section>',
    proactiveAuditDisclosure(`证据引用（${evidenceRefs.length}）`, evidenceRefs, [], "按需展开"),
    proactiveAuditDisclosure("证据快照", row.evidence_snapshot, {}, "按需展开"),
    proactiveAuditDisclosure("约束与预算", row.budget_snapshot, {}, "按需展开"),
    proactiveAuditDisclosure("执行生命周期", {
      ...
      interventionUpdatedAt: row.intervention_updated_at || "",
      currentPhase: currentLifecycle.phase || "",
      currentLevel: row.current_policy_level || "",
      fadeLevel: Number(row.current_fade_level || 0),
      lifecycleUpdatedAt: row.policy_state_updated_at || ""
    }, {}, "按需展开"),
    proactiveAuditDisclosure("后续结果", outcome, {}, "按需展开")
  ].join("");
}

function selectProactiveDecision(decisionId) {
  selectedProactiveDecisionId = String(decisionId || "");
  if (selectedProactiveDecisionId) {
    sessionStorage.setItem("cq_proactive_selected_decision", selectedProactiveDecisionId);
  }
  renderProactiveAudit(cachedProactiveStudyData);
  Array.from(document.querySelectorAll("[data-proactive-audit-id]"))
    .find((node) => node.dataset.proactiveAuditId === selectedProactiveDecisionId)
    ?.scrollIntoView({ block: "nearest" });
}

const proactiveOutcomeStageLabels = {
  near_transfer: "近迁移",
  representation_transfer: "跨表征迁移",
  retention: "延迟保持"
};

function proactiveOutcomeStageLabel(value = "") {
  return proactiveOutcomeStageLabels[value] || value || "未标记阶段";
}

function proactiveOutcomePercent(value) {
  const number = Number(value);
  if (!Number.isFinite(number)) return "-";
  return `${(number * 100).toFixed(1).replace(/\.0$/, "")}%`;
}

function proactiveDuration(value) {
  const seconds = Math.max(0, Math.round(Number(value || 0) / 1000));
  if (seconds < 60) return `${seconds} 秒`;
  return `${Math.floor(seconds / 60)} 分 ${seconds % 60} 秒`;
}

function proactiveArmList(data = {}) {
  return Array.from(new Set([
    ...safeRows(data.policy?.experimentArms),
    ...safeRows(data.assignments).map((row) => row.condition),
    ...safeRows(data.summary?.conditions).map((row) => row.condition),
    ...safeRows(data.outcomes?.summary).map((row) => row.condition)
  ].filter(Boolean)));
}

function syncProactiveConditionOptions(data = {}) {
  const select = document.getElementById("proactive-condition-filter");
  if (!select) return;
  const arms = proactiveArmList(data);
  select.replaceChildren();
  const all = document.createElement("option");
  all.value = "";
  all.textContent = "全部条件";
  select.appendChild(all);
  arms.forEach((arm) => {
    const option = document.createElement("option");
    option.value = arm;
    option.textContent = proactiveConditionLabel(arm);
    select.appendChild(option);
  });
  if (proactiveConditionFilter && !arms.includes(proactiveConditionFilter)) {
    proactiveConditionFilter = "";
    sessionStorage.removeItem("cq_proactive_condition");
  }
  select.value = proactiveConditionFilter;
}

function proactiveConditionBadgeClass(policy = {}, condition = "") {
  const profile = policy.armProfiles?.[condition];
  if (profile && typeof profile.display === "boolean") {
    return profile.display ? "badge-green" : "badge-blue";
  }
  return condition === "treatment" ? "badge-green" : "badge-blue";
}

function renderProactiveOutcomes(outcomes = {}, policy = {}) {
  const rows = safeRows(outcomes.rows);
  const sessionSummaryRows = safeRows(outcomes.sessionSummary);
  const summaryRows = safeRows(outcomes.summary).sort((left, right) => {
    const stageOrder = Object.keys(proactiveOutcomeStageLabels);
  ...
  const overview = document.getElementById("proactive-outcome-overview");
  const stageSubmissionCount = (stage) => summaryRows
    .filter((row) => row.stage === stage)
    .reduce((sum, row) => sum + Number(row.submissions || 0), 0);
  const sessionStatusCount = (status) => sessionSummaryRows
    .filter((row) => row.status === status)
    .reduce((sum, row) => sum + Number(row.sessions || 0), 0);
  if (overview) {
    overview.innerHTML = [
      ["全部测量提交", Number(outcomes.total || rows.length), `量表 ${outcomes.instrumentVersion || "未标记"}`],
      ...(["v14-c3-retention-v4", "functions-limits-retention-r1", "functions-limits-retention-r2", "functions-limits-retention-r3"].includes(outcomes.instrumentVersion) ? [] : [
        ["近迁移（历史）", stageSubmissionCount("near_transfer"), "新情境应用"],
        ["跨表征迁移（历史）", stageSubmissionCount("representation_transfer"), "图像、语言与符号转换"]
      ]),
      ["保持测量", stageSubmissionCount("retention"), `延迟 ${Number(outcomes.retentionDelayHours || 0)} 小时`],
      [
        "测量会话",
        Number(outcomes.sessionTotal || 0),
        `进行中 ${sessionStatusCount("active")} · 已退出 ${sessionStatusCount("withdrawn")}`
      ]
    ].map(([label, value, detail]) => `
      <div class="proactive-outcome-overview-item">
        <span>${esc(label)}</span><strong>${esc(value)}</strong><small>${esc(detail)}</small>
      </div>
    `).join("");
  }

  const summaryBody = document.querySelector("#table-proactive-outcome-summary tbody");
  if (summaryBody) {
    summaryBody.innerHTML = summaryRows.length
      ? summaryRows.map((row) => `
        <tr>
          <td><strong>${esc(proactiveOutcomeStageLabel(row.stage))}</strong><br><small>${esc(row.instrument_version || "历史版本未标记")}</small></td>
          <td><span class="badge ${proactiveConditionBadgeClass(policy, row.condition)}">${esc(proactiveConditionLabel(row.condition))}</span></td>
          <td>${Number(row.participants || 0)}</td>
          <td>${Number(row.submissions || 0)}</td>
          <td>${esc(proactiveOutcomePercent(row.mean_accuracy))}</td>
          <td>${esc(proactiveDuration(row.mean_duration_ms))}</td>
        </tr>
      `).join("")
      : "<tr><td colspan='6'>尚无独立学习结果。</td></tr>";
    syncTableDensity(summaryBody.closest("table"));
  }
  const summaryStatus = document.getElementById("proactive-outcome-summary-status");
  if (summaryStatus) summaryStatus.textContent = `${summaryRows.length} 个阶段×版本×实验臂组合`;

  const attemptsBody = document.querySelector("#table-proactive-outcome-attempts tbody");
  if (attemptsBody) {
    attemptsBody.innerHTML = rows.length
      ? rows.map((row) => `
        <tr>
          <td style="white-space:nowrap;font-size:0.78rem;">${esc((row.submitted_at || "").replace("T", " ").slice(0, 19))}</td>
          <td><strong>${esc(row.nickname || "")}</strong><br><span class="muted">${esc(row.user_id || "")}</span></td>
          <td><span class="badge ${proactiveConditionBadgeClass(policy, row.condition)}">${esc(proactiveConditionLabel(row.condition))}</span></td>
          <td>${esc(proactiveOutcomeStageLabel(row.stage))}</td>
          <td>${Number(row.score || 0)} / ${Number(row.max_score || 0)}</td>
          <td>${esc(proactiveOutcomePercent(row.accuracy))}</td>
          <td>${esc(proactiveDuration(row.duration_ms))}</td>
          <td>第 ${Math.max(1, Number(row.learning_generation || 1))} 次</td>
        </tr>
      `).join("")
      : "<tr><td colspan='8'>当前筛选条件下暂无测量提交。</td></tr>";
    syncTableDensity(attemptsBody.closest("table"));
  }
  const attemptsStatus = document.getElementById("proactive-outcome-attempts-status");
  if (attemptsStatus) {
    attemptsStatus.textContent = Number(outcomes.total || 0) > rows.length
      ? `最近 ${rows.length} / ${Number(outcomes.total || 0)} 条`
      : `${rows.length} 条`;
  }
}

const proactiveDataQualityLabels = {
  courseware_metadata_missing: "课件实验元数据缺失",
  courseware_experiment_mismatch: "课件实验编号不一致",
  courseware_group_mismatch: "课件事件分组不一致",
  courseware_assignment_version_mismatch: "课件分配版本不一致",
  courseware_generation_mismatch: "课件学习代次不一致",
  courseware_protocol_mismatch: "课件协议指纹不一致",
  courseware_semantic_id_missing: "课件语义事件 ID 缺失",
  courseware_semantic_id_duplicate: "课件语义事件 ID 重复",
  courseware_event_order_anomaly: "课件事件顺序异常",
  courseware_event_chain_missing: "课件挑战事件链缺失",
  courseware_events_missing: "完成后测但无课件语义记录",
  repeated_learning_generation: "存在重复学习代次",
  decision_group_mismatch: "主动决策分组不一致",
  decision_assignment_version_mismatch: "主动决策分配版本不一致",
  outcome_group_mismatch: "结果测量分组不一致",
  outcome_protocol_mismatch: "结果测量协议不一致"
};

function proactivePhaseAnalysisCell(phase = {}) {
  if (!Number(phase.submitted || 0)) return '<span class="muted">未提交</span>';
  const completion = phase.completed ? "已完成" : "未完整";
  const pending = Number(phase.pending || 0)
    ? ` · 待评分 ${Number(phase.pending || 0)}`
    : "";
  return `${Number(phase.score || 0)} / ${Number(phase.max_score || 0)}<br><span class="muted">${esc(completion)} · ${Number(phase.score_rate || 0)}%${pending}</span>`;
}

function proactiveOutcomeAnalysisCell(outcome = {}) {
  const labels = {
    not_started: "未开始",
    not_applicable: "本版本不适用",
    active: "作答中",
    paused: "已暂离",
    submitted: "已提交",
    skipped: "永久跳过",
  ...
  const label = labels[outcome.status] || outcome.status || "未开始";
  const score = outcome.status === "submitted"
    ? `${Number(outcome.score || 0)} / ${Number(outcome.max_score || 0)} · ${Number(outcome.accuracy || 0)}%`
    : outcome.withdrawal_reason || "";
  return `${esc(label)}${score ? `<br><span class="muted">${esc(score)}</span>` : ""}`;
}

function proactiveRetentionWindowLabel(windowState = {}) {
  const labels = {
    not_scheduled: "尚未排期",
    waiting_for_transfer: "等待迁移测量",
    waiting: "等待开放",
    open: "可作答",
    reminder_due: "提醒待发送",
    closing_soon: "即将超窗",
  ...
  const reminder = safeRows(windowState.due_reminder_hours).length
    ? ` · 待提醒 ${safeRows(windowState.due_reminder_hours).join("、")} 小时`
    : safeRows(windowState.shown_reminder_hours).length
      ? ` · 已提醒 ${safeRows(windowState.shown_reminder_hours).join("、")} 小时`
      : "";
  return `${labels[windowState.status] || windowState.status || "未知"}${reminder}`;
}

function renderProactiveExperimentAnalysis(data = {}, policy = {}) {
  const rows = safeRows(data.participantAnalysis);
  const challenges = safeRows(data.coursewareChallenges);
  const summary = data.analysisSummary || {};
  const quality = data.dataQuality || {};
  const overview = document.getElementById("proactive-analysis-overview");
  if (overview) {
    const values = [
      ["已固定分组", Number(summary.participants || rows.length), "学生不能自行选择实验臂"],
      ["课件语义事件", Number(summary.courseware_events || 0), "服务端绑定实验元数据"],
      ["挑战尝试", Number(summary.challenge_attempts || 0), `${challenges.length} 个稳定挑战 ID`],
      ["数据完整", Number(summary.data_complete || 0), "当前未发现质量标记"],
      ["需要复核", Number(summary.needs_review || 0), `待学习 ${Number(summary.pending_learning || 0)} 人`]
    ];
    overview.innerHTML = values.map(([label, value, note]) => `
      <div class="proactive-outcome-overview-item">
        <span>${esc(label)}</span><strong>${Number(value || 0)}</strong><small>${esc(note)}</small>
      </div>
    `).join("");
  }

  const qualityStatus = document.getElementById("proactive-data-quality-status");
  const qualitySignals = safeRows(quality.signals);
  if (qualityStatus) {
    qualityStatus.textContent = quality.status === "critical"
      ? "存在阻断性数据问题"
  ...
  const qualityList = document.getElementById("proactive-data-quality-signals");
  if (qualityList) {
    qualityList.innerHTML = qualitySignals.length
      ? qualitySignals.map((signal) => `
        <div class="proactive-integrity-signal is-${esc(signal.severity || "warning")}">
          <div>
            <strong>${esc(proactiveDataQualityLabels[signal.id] || signal.id)}</strong>
            <p>${Number(signal.participants || 0)} 名学生 · ${Number(signal.events || 0)} 次记录</p>
          </div>
          <b>${Number(signal.events || 0)}</b>
        </div>
      `).join("")
      : '<div class="proactive-integrity-signal is-ok"><div><strong>未发现课件数据链异常</strong><p>仍需结合样本量和实际学习进度解释。</p></div><b>0</b></div>';
  }

  const participantStatus = document.getElementById("proactive-participant-analysis-status");
  if (participantStatus) participantStatus.textContent = `${rows.length} 人；一人一行`;
  const participantBody = document.querySelector("#table-proactive-participant-analysis tbody");
  if (participantBody) {
    participantBody.innerHTML = rows.length
      ? rows.map((row) => {
          const courseware = row.courseware || {};
          const proactive = row.proactive || {};
          const flags = safeRows(row.data_quality?.flags);
          const qualityLabel = row.data_quality?.status === "complete"
            ? "完整"
            : row.data_quality?.status === "review"
              ? "需复核"
              : "待学习";
          return `<tr>
            <td><strong>${esc(row.nickname || "未命名学生")}</strong><br><span class="muted">${esc(row.user_id || "")}</span><br><span class="badge ${proactiveConditionBadgeClass(policy, row.condition)}">${esc(proactiveConditionLabel(row.condition))}</span></td>
            <td>${esc(proactiveStratumLabel(row.stratum))}<br><span class="muted">第 ${Number(row.analysis_learning_generation || 1)} 次数据</span></td>
            <td>${proactivePhaseAnalysisCell(row.pretest)}</td>
            <td>${proactivePhaseAnalysisCell(row.formative)}</td>
            <td>${proactivePhaseAnalysisCell(row.posttest)}</td>
            <td>首次独立 ${Number(courseware.first_independent_correct || 0)} / ${Number(courseware.challenge_count || 0)}<br><span class="muted">总尝试 ${Number(courseware.challenge_attempt_accuracy || 0)}% · 重试 ${Number(courseware.retry_challenges || 0)} · 重置 ${Number(courseware.reset_events || 0)}</span></td>
            <td>展示 ${Number(proactive.shown || 0)} · 接受 ${Number(proactive.accepted || 0)}<br><span class="muted">稍后 ${Number(proactive.snoozed || 0)} · 换帮助 ${Number(proactive.alternative_requested || 0)} · 关闭知识点 ${Number(proactive.scope_mutes || 0)}</span></td>
            <td>${proactiveOutcomeAnalysisCell(row.outcomes?.near_transfer)}</td>
            <td>${proactiveOutcomeAnalysisCell(row.outcomes?.representation_transfer)}</td>
            <td>${proactiveOutcomeAnalysisCell(row.outcomes?.retention)}<br><span class="muted">${esc(proactiveRetentionWindowLabel(row.retention_window))}</span></td>
            <td><strong>${esc(qualityLabel)}</strong>${flags.length ? `<br><span class="muted">${esc(flags.map((flag) => proactiveDataQualityLabels[flag] || flag).join("；"))}</span>` : ""}</td>
          </tr>`;
        }).join("")
      : "<tr><td colspan='11'>尚无已固定分组的参与者。</td></tr>";
    syncTableDensity(participantBody.closest("table"));
  }

  const challengeStatus = document.getElementById("proactive-challenge-analysis-status");
  if (challengeStatus) challengeStatus.textContent = `${challenges.length} 个挑战`;
  const challengeBody = document.querySelector("#table-proactive-challenge-analysis tbody");
  if (challengeBody) {
    challengeBody.innerHTML = challenges.length
      ? challenges.map((row) => {
          const qualityCount = Number(row.metadata_issues || 0)
            + Number(row.duplicate_events || 0)
            + Number(row.order_anomalies || 0)
            + Number(row.missing_event_chains || 0);
          return `<tr>
            <td><strong>${esc(row.knowledge_point || row.knowledge_point_id || "知识点")}</strong><br><span class="muted">${esc(row.challenge_id || "未命名挑战")}</span></td>
            <td>${Number(row.participants || 0)}</td>
            <td>${Number(row.attempts || 0)}<br><span class="muted">漏接 ${Number(row.missed_attempts || 0)} · 未归类 ${Number(row.unclassified_attempts || 0)}</span></td>
            <td>${Number(row.first_independent_correct || 0)} · ${Number(row.first_independent_accuracy || 0)}%</td>
            <td>${Number(row.supported_correct || 0)} / ${Number(row.supported_attempts || 0)} · ${Number(row.supported_accuracy || 0)}%</td>
            <td>${Number(row.correct_attempts || 0)} / ${Number(row.attempts || 0)} · ${Number(row.total_attempt_accuracy || 0)}%</td>
            <td>${Number(row.retry_participants || 0)} · ${Number(row.retry_rate || 0)}%</td>
            <td>${Number(row.reset_participants || 0)} · ${Number(row.reset_rate || 0)}%</td>
            <td>${Number(row.challenge_completed_participants || 0)} · ${Number(row.challenge_completion_rate || 0)}%</td>
            <td>${Number(row.scene_completed_participants || 0)} · ${Number(row.scene_completion_rate || 0)}%</td>
            <td>${qualityCount ? `<strong class="text-warn">${qualityCount}</strong>` : "0"}</td>
          </tr>`;
        }).join("")
      : "<tr><td colspan='11'>尚无课件挑战结果。</td></tr>";
    syncTableDensity(challengeBody.closest("table"));
  }
}

function renderProactiveStudy(data = {}) {
  cachedProactiveStudyData = data;
  cachedProactiveDecisionRows = safeRows(data.rows);
  const summary = data.summary || {};
  const assignments = safeRows(data.assignments);
  const participation = safeRows(data.participation);
  const policy = data.policy || {};
  const arms = proactiveArmList(data);
  const assignmentByArm = Object.fromEntries(
    arms.map((arm) => [
      arm,
      assignments
        .filter((row) => row.condition === arm)
        .reduce((sum, row) => sum + Number(row.participants || 0), 0)
    ])
  );
  const assignmentCount = Object.values(assignmentByArm)
  ...
  const withdrawnParticipants = participation
    .filter((row) => row.status === "withdrawn")
    .reduce((sum, row) => sum + Number(row.participants || 0), 0);
  const terminalResponses = Number(summary.accepted || 0)
    + Number(summary.dismissed || 0)
    + Number(summary.ignored || 0)
    + Number(summary.snoozed || 0)
    + Number(summary.alternative_requested || 0);
  const acceptanceRate = terminalResponses
    ? Math.round((Number(summary.accepted || 0) / terminalResponses) * 100)
    : 0;
  const supportCounts = [
    Number(summary.l1_self_explanations || 0),
    Number(summary.l2_concept_hints || 0),
    Number(summary.l3_subgoal_decompositions || 0),
    Number(summary.l4_next_step_hints || 0),
    Number(summary.l5_partial_examples || 0)
  ];
  const autonomy = summary.autonomy || {};
  syncProactiveConditionOptions(data);
  renderProactiveGovernance(data);
  renderProactiveExperimentAnalysis(data, policy);
  renderProactiveOutcomes(data.outcomes || {}, policy);

  const metrics = document.getElementById("proactive-study-metrics");
  if (metrics) {
    const armBreakdown = arms.length
      ? `<div class="metric-breakdown">${arms.map((arm) => (
          `<span><b>${esc(proactiveConditionLabel(arm))}</b><em>${Number(assignmentByArm[arm] || 0)}</em></span>`
        )).join("")}</div>`
      : '<div class="metric-empty">尚未配置实验臂</div>';
    metrics.innerHTML = `
      <div class="metric-card highlight detail"><div><div class="label">已分组学生</div><div class="value">${assignmentCount}</div></div>${armBreakdown}</div>
      <div class="metric-card"><div class="label">研究参与状态</div><div class="value">${enrolledParticipants}</div><div class="sub">当前在研 · 已退出 ${withdrawnParticipants} · assignment 保留用于 ITT</div></div>
      <div class="metric-card"><div class="label">决策窗口</div><div class="value">${Number(summary.decisions || 0)}</div><div class="sub">${Number(summary.participants || 0)} 名学生产生证据</div></div>
      <div class="metric-card warn"><div class="label">政策判断应介入</div><div class="value">${Number(summary.policy_interventions || 0)}</div><div class="sub">包含未实际展示的反事实判断</div></div>
      <div class="metric-card good"><div class="label">实际签发展示</div><div class="value">${Number(summary.delivered_interventions || 0)}</div><div class="sub">展示实验臂：${esc(safeRows(policy.displayArms).map(proactiveConditionLabel).join("、") || "无")}</div></div>
      <div class="metric-card"><div class="label">接受率</div><div class="value">${acceptanceRate}%</div><div class="sub">接受 ${Number(summary.accepted || 0)} · 关闭 ${Number(summary.dismissed || 0)} · 忽略 ${Number(summary.ignored || 0)} · 稍后 ${Number(summary.snoozed || 0)} · 换帮助 ${Number(summary.alternative_requested || 0)}</div></div>
      <div class="metric-card"><div class="label">学生自主权动作</div><div class="value">${Number(summary.snoozed || 0) + Number(summary.alternative_requested || 0) + Number(autonomy.scope_mutes || 0)}</div><div class="sub">稍后 ${Number(summary.snoozed || 0)} · 换帮助 ${Number(summary.alternative_requested || 0)} · 关闭知识点 ${Number(autonomy.scope_mutes || 0)} · 已恢复 ${Number(autonomy.scope_restores || 0)}</div></div>
      <div class="metric-card detail"><div><div class="label">L1-L5 支架动作</div><div class="value">${supportCounts.reduce((sum, value) => sum + value, 0)}</div></div><div class="metric-breakdown">${supportCounts.map((value, index) => `<span><b>L${index + 1}</b><em>${value}</em></span>`).join("")}</div></div>
      <div class="metric-card"><div class="label">换表征执行</div><div class="value">${Number(summary.representation_switches || 0)}</div><div class="sub">只在学生明确确认且实验臂允许时执行</div></div>
      <div class="metric-card good"><div class="label">检查成功结束帮助</div><div class="value">${Number(summary.independent_success_exits || 0)}</div><div class="sub">含支架后检查或课件结果 · 持续卡点 ${Number(summary.continued_friction || 0)} · 静音知识点 ${Number(autonomy.current_muted_scopes || 0)}</div></div>
      <div class="metric-card warn"><div class="label">测量保护阻断</div><div class="value">${Number(summary.protected_blocks || 0)}</div><div class="sub">受保护测量阶段保持静默</div></div>
    `;
  }

  const policySummary = document.getElementById("proactive-policy-summary");
  if (policySummary) {
    policySummary.textContent = [
      `模式 ${String(policy.mode || "off").toUpperCase()}`,
      `实验 ${policy.experimentId || "未配置"}`,
      `实验臂 ${arms.map(proactiveConditionLabel).join(" / ") || "未配置"}`,
      `政策 ${policy.policyVersion || "未配置"}`,
      `契约 ${policy.contractVersion || "未配置"}`,
      `Adapter ${policy.adapterVersion || "未配置"}`
    ].join(" · ");
  }
  const pageStatus = document.getElementById("proactive-study-page-status");
  if (pageStatus) {
    pageStatus.textContent = data.total > cachedProactiveDecisionRows.length
      ? `显示最近 ${cachedProactiveDecisionRows.length} / ${data.total} 条`
      : `${Number(data.total || 0)} 条决策`;
  }

  const tbody = document.querySelector("#table-proactive-decisions tbody");
  if (!tbody) {
    renderProactiveAudit(data);
    return;
  }
  const table = tbody.closest("table");
  if (!cachedProactiveDecisionRows.length) {
    tbody.innerHTML = "<tr><td colspan='10'>当前筛选条件下暂无主动政策决策。</td></tr>";
    syncTableDensity(table);
    renderProactiveAudit(data);
    return;
  }
  tbody.innerHTML = cachedProactiveDecisionRows.map((row) => {
    const conditionClass = proactiveConditionBadgeClass(policy, row.condition);
    const decisionClass = row.decision === "intervene" ? "badge-amber" : "badge-blue";
    const deliveryClass = row.delivery_decision === "intervene" ? "badge-green" : "badge-blue";
    const resolution = row.resolution || row.intervention_status || "";
    const resolutionText = proactiveResolutionLabels[resolution]
      || (row.delivery_decision === "stay_silent" ? "未展示" : "待处理");
    const latency = Number(row.latency_ms || 0);
    return `<tr>
      <td style="white-space:nowrap;font-size:0.78rem;">${esc((row.created_at || "").replace("T", " ").slice(0, 19))}<br><span class="muted">第 ${esc(proactiveLearningGeneration(row))} 次学习</span></td>
      <td><strong>${esc(row.nickname || "")}</strong><br><span class="muted">${esc(row.user_id || "")}</span></td>
      <td><span class="badge ${conditionClass}">${esc(proactiveConditionLabel(row.condition))}</span><br><span class="muted">${esc(proactiveStratumLabel(row.stratum))}</span></td>
      <td>${esc(chapterName(row.chapter_id))}<br><span class="muted">${esc(moduleName(row.unit_id, row.unit_id || ""))}</span></td>
      <td>${esc(proactiveCandidateLabels[row.candidate_kind] || row.candidate_kind || "")}<br><span class="muted">${esc(AdminPresentation.sceneTypeLabel(row.scene_type || ""))}</span></td>
      <td><span class="badge ${decisionClass}">${esc(proactiveDecisionLabel(row.decision))}</span><br><span class="badge ${deliveryClass}">${esc(proactiveDeliveryLabel(row.delivery_decision))}</span></td>
      <td>${esc(proactiveActionLabels[row.action] || row.action || "")}<br><span class="muted">${esc(row.support_level || "L0")}</span></td>
      <td class="decision-facts"><span>H ${proactiveScore(row.help_need_score)}</span><span>I ${proactiveScore(row.interruptibility_score)}</span><span>U ${proactiveScore(row.uncertainty_score)}</span></td>
      <td>${esc(proactiveReasonText(row.reason_codes))}</td>
      <td>${esc(resolutionText)}${latency ? `<br><span class="muted">${latency} ms</span>` : ""}</td>
    </tr>`;
  }).join("");
  syncTableDensity(table);
  renderProactiveAudit(data);
...
async function reloadProactiveStudy() {
  const status = document.getElementById("proactive-study-page-status");
  if (status) status.textContent = "正在刷新...";
  try {
    const data = await fetchProactiveStudy();
    renderProactiveStudy(data);
  } catch (error) {
    if (status) status.textContent = error.message || "加载失败";
  }
}

async function updateProactiveRuntime(status) {
  const resultNode = document.getElementById("proactive-runtime-result");
  const reasonInput = document.getElementById("proactive-pause-reason");
  const operatorInput = document.getElementById("proactive-runtime-operator");
  const reason = reasonInput?.value.trim() || "";
  if (status === "paused" && !reason) {
    if (resultNode) resultNode.textContent = "暂停前需要填写原因。";
    reasonInput?.focus();
    return false;
  }
  const response = await fetch(`${API_BASE}/api/admin/research/proactive-runtime`, {
    method: "POST",
    headers: adminRequestHeaders({ "Content-Type": "application/json" }),
    body: JSON.stringify({
      experimentId: cachedProactiveStudyData.policy?.experimentId || "",
      status,
      reason,
      updatedBy: operatorInput?.value.trim() || "研究管理员"
    })
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(payload.message || `运行状态更新失败（HTTP ${response.status}）`);
  }
  if (resultNode) {
    resultNode.textContent = status === "paused"
      ? `实验已暂停；收束 ${Number(payload.data?.closedInterventions || 0)} 条未决提示。`
      : "实验已按原冻结协议恢复。";
  }
  await reloadProactiveStudy();
  return true;
}

async function fetchAllProactiveDecisionRows() {
  const pageSize = 1000;
  const rows = [];
  let offset = 0;
  let total = Infinity;
  while (offset < total) {
    const page = await fetchProactiveStudy(undefined, { limit: pageSize, offset });
    rows.push(...page.rows);
    total = page.total;
...
async function fetchAllProactiveOutcomeRows() {
  const pageSize = 1000;
  const rows = [];
  let offset = 0;
  let total = Infinity;
  while (offset < total) {
    const page = await fetchProactiveStudy(undefined, {
      limit: pageSize,
      offset
    });
    const outcomeRows = safeRows(page.outcomes?.rows);
    rows.push(...outcomeRows);
    total = Number(page.outcomes?.total || 0);
    if (!outcomeRows.length) break;
    offset += outcomeRows.length;
  }
  return rows;
}

async function fetchAllProactiveOutcomeSessionRows() {
  const pageSize = 1000;
  const rows = [];
  let offset = 0;
  let total = Infinity;
  while (offset < total) {
    ...
    const sessionRows = safeRows(page.outcomes?.sessions);
    rows.push(...sessionRows);
    total = Number(page.outcomes?.sessionTotal || 0);
    if (!sessionRows.length) break;
    offset += sessionRows.length;
  }
  return rows;
}

document.addEventListener("click", (event) => {
  const auditEvent = event.target.closest?.("[data-proactive-audit-id]");
  if (!auditEvent) return;
  selectProactiveDecision(auditEvent.dataset.proactiveAuditId);
});

document.getElementById("proactive-audit-load-all")?.addEventListener("click", async (event) => {
  const button = event.currentTarget;
  const previousText = button.textContent;
  button.disabled = true;
  button.textContent = "正在加载...";
  try {
    const rows = await fetchAllProactiveDecisionRows();
    proactiveAuditAllLoaded = true;
    renderProactiveStudy({ ...cachedProactiveStudyData, rows, total: rows.length, limit: rows.length, offset: 0 });
  } catch (error) {
    const status = document.getElementById("proactive-audit-status");
    if (status) status.textContent = error.message || "加载失败";
  } finally {
...
document.getElementById("proactive-runtime-toggle")?.addEventListener("click", async (event) => {
  const button = event.currentTarget;
  const nextStatus = button.dataset.nextStatus === "active" ? "active" : "paused";
  const prompt = nextStatus === "paused"
    ? "暂停后，系统会立即停止新主动介入并收束所有未决提示。确认暂停吗？"
    : "恢复后，系统会继续执行当前已冻结协议，不会重新分组。确认恢复吗？";
  if (!window.confirm(prompt)) return;
  const previousText = button.textContent;
  button.disabled = true;
  button.setAttribute("aria-busy", "true");
  button.textContent = nextStatus === "paused" ? "正在暂停…" : "正在恢复…";
  try {
    await updateProactiveRuntime(nextStatus);
  } catch (error) {
    const resultNode = document.getElementById("proactive-runtime-result");
    if (resultNode) resultNode.textContent = error.message || "运行状态更新失败。";
  } finally {
    button.disabled = false;
    button.removeAttribute("aria-busy");
    if (button.textContent.includes("正在")) button.textContent = previousText;
  }
});
...
// ---- Tab switching ----
function revealActiveAdminTab(tab = document.querySelector(".tab.active")) {
...
document.querySelectorAll(".tab").forEach(tab => {
  tab.addEventListener("click", () => {
    ...
    revealActiveAdminTab(tab);
  });
});

window.addEventListener("resize", () => {
  window.requestAnimationFrame(() => revealActiveAdminTab());
});
...
const proactiveConditionSelect = document.getElementById("proactive-condition-filter");
const proactiveDecisionSelect = document.getElementById("proactive-decision-filter");
if (proactiveConditionSelect) proactiveConditionSelect.value = proactiveConditionFilter;
if (proactiveDecisionSelect) proactiveDecisionSelect.value = proactiveDecisionFilter;
proactiveConditionSelect?.addEventListener("change", (event) => {
  proactiveConditionFilter = event.target.value;
  sessionStorage.setItem("cq_proactive_condition", proactiveConditionFilter);
  proactiveAuditAllLoaded = false;
  reloadProactiveStudy();
});
proactiveDecisionSelect?.addEventListener("change", (event) => {
  proactiveDecisionFilter = event.target.value;
  sessionStorage.setItem("cq_proactive_decision", proactiveDecisionFilter);
  proactiveAuditAllLoaded = false;
  reloadProactiveStudy();
});
...
function proactiveDecisionCsvRows(data) {
  const rows = [[
    "决策ID", "干预ID", "时间", "学习代次", "学生", "用户ID", "实验", "条件", "队列", "前测层级",
    "研究参与状态", "同意版本", "同意时间", "退出时间", "退出原因",
    "章节", "单元", "策略范围", "场景", "候选信号", "政策模式", "政策版本", "契约版本", "Adapter版本",
    "政策判断", "实际呈现", "动作", "支架强度", "HelpNeed", "可打断性", "不确定性",
    "理由码", "证据引用", "证据快照", "预算快照", "干预状态", "展示时间", "反馈", "反馈时间", "反馈耗时毫秒",
    "后续结果", "后续证据来源", "后续证据事件", "验证题ID", "验证阶段",
    "当前生命周期", "当前强度", "渐退次数", "状态更新时间",
    "支架检查版本", "检查证据范围", "检查子技能",
    "课件匹配版本", "课件证据采用", "课件匹配理由", "课件证据范围", "课件证据子技能", "课件证据事件ID"
  ]];
  safeRows(data).forEach((row) => {
    const attempt = row.followup_outcome?.independentAttempt || {};
    const matching = row.evidence_snapshot?.coursewareVerificationMatch || {};
    rows.push([
      row.id || "",
      row.intervention_id || "",
      row.created_at || "",
      proactiveLearningGeneration(row),
      row.nickname || "",
      row.user_id || "",
      row.experiment_id || "",
      row.condition || "",
      ...
      row.participation_consent_version || "",
      row.participation_consented_at || "",
      row.participation_withdrawn_at || "",
      row.participation_withdrawal_reason || "",
      row.chapter_id || "",
      row.unit_id || "",
      ...
      row.support_level || "",
      row.help_need_score ?? "",
      row.interruptibility_score ?? "",
      row.uncertainty_score ?? "",
      JSON.stringify(row.reason_codes || []),
      JSON.stringify(row.evidence_refs || []),
      JSON.stringify(row.evidence_snapshot || {}),
      JSON.stringify(row.budget_snapshot || {}),
      row.intervention_status || "",
      row.shown_at || "",
      row.resolution || "",
      row.resolved_at || "",
      row.latency_ms || 0,
      JSON.stringify(row.followup_outcome || {}),
      attempt.source || "",
      attempt.eventType || "",
      attempt.checkId || "",
      attempt.stage || "",
      JSON.stringify(row.current_lifecycle || {}),
      row.current_policy_level || "",
      row.current_fade_level || 0,
      row.policy_state_updated_at || "",
      attempt.instrumentVersion || "",
      attempt.evidenceScope || "",
      attempt.subSkill || "",
      matching.matchingVersion || "",
      matching.eligible === true ? "是" : matching.eligible === false ? "否" : "",
      matching.reasonCode || "",
      matching.evidenceScope || "",
      matching.subSkill || "",
      matching.eventId || ""
    ]);
  });
...
function proactiveShadowCodingCsvRows(data) {
  const rows = [[
    "decision_id", "timestamp", "user_id", "learning_generation", "condition", "stratum",
    "participation_status", "participation_consent_version",
    "participation_consented_at", "participation_withdrawn_at",
    "participation_withdrawal_reason",
    "chapter_id", "unit_id", "scope_key", "scene_type", "candidate_kind",
    "help_need_score", "interruptibility_score", "uncertainty_score",
    "evidence_refs", "evidence_snapshot", "reason_codes",
    "policy_decision", "delivery_decision", "action", "support_level",
    "intervention_status", "student_resolution",
    "reviewer_id", "help_need_label", "interruptibility_label",
    "should_intervene", "false_positive", "notes"
  ]];
  safeRows(data)
    .filter((row) => row.policy_mode === "shadow")
    .forEach((row) => {
      rows.push([
        row.id || "",
        row.created_at || "",
...
function proactiveOutcomeCsvRows(data) {
  const rows = [[
    "测量ID", "提交时间", "开始时间", "用时毫秒", "学生", "用户ID",
    "实验", "实验臂", "队列", "研究参与状态", "同意版本", "同意时间", "退出时间", "退出原因",
    "章节", "测量阶段", "量表版本",
    "得分", "满分", "正确率", "学习代次", "逐题结果", "原始作答"
  ]];
  safeRows(data).forEach((row) => {
    rows.push([
      row.id || "",
      ...
      Math.max(1, Number(row.learning_generation || 1)),
      JSON.stringify(row.item_results || []),
...
function proactiveOutcomeSessionCsvRows(data) {
  const rows = [[
    "会话ID", "开始时间", "结束时间", "会话状态", "退出原因",
    "测量提交ID", "学生", "用户ID", "实验", "实验臂", "队列",
    "研究参与状态", "同意版本", "同意时间", "研究退出时间", "研究退出原因",
    "章节", "测量阶段", "量表版本", "协议指纹", "学习代次", "最后更新时间"
  ]];
  safeRows(data).forEach((row) => {
    rows.push([
      row.id || "",
...
function proactiveParticipantAnalysisCsvRows(data, protocol = null) {
  const rows = [[
    "学生", "用户ID", "实验", "固定实验臂", "队列", "前测层级", "分配版本", "分配时间",
    "参与状态", "同意时间", "退出时间", "当前学习代次", "主分析学习代次", "学习代次数",
    "前测已交", "前测应交", "前测得分", "前测满分", "前测得分率%", "前测完成时间",
    "形成测已交", "形成测应交", "形成测得分", "形成测满分", "形成测得分率%", "形成测待评分", "形成测完成时间",
    "后测已交", "后测应交", "后测得分", "后测满分", "后测得分率%", "后测待评分", "后测完成时间",
    "课件语义事件", "打开课件数", "完成场景数", "挑战数", "挑战尝试", "挑战正确尝试", "总尝试正确率%",
    "首次独立正确", "首次独立正确率%", "独立尝试", "独立正确", "独立正确率%",
    "提示或重置后尝试", "提示或重置后正确", "提示或重置后正确率%", "重试挑战", "重试率%", "提示事件", "重置事件", "课件时长毫秒",
    "政策决策", "政策判断应介入", "实际签发", "实际展示", "接受", "关闭", "忽略", "稍后", "换帮助", "关闭知识点", "恢复知识点",
    "L1", "L2", "L3", "L4", "L5",
    "近迁移状态", "近迁移得分", "近迁移满分", "近迁移正确率%", "近迁移暂停次数", "近迁移暂停毫秒",
    "跨表征状态", "跨表征得分", "跨表征满分", "跨表征正确率%", "跨表征暂停次数", "跨表征暂停毫秒",
    "保持测状态", "保持测得分", "保持测满分", "保持测正确率%", "保持测暂停次数", "保持测暂停毫秒",
    "保持窗口状态", "保持测开放时间", "保持测关闭时间", "章后经过小时", "已提醒节点", "待提醒节点", "即将超窗", "已超窗",
    "数据质量状态", "数据质量问题数", "数据质量标记", "学习代次列表",
    "前测状态", "前测待评分", "形成测状态", "后测状态",
    "协议指纹", "题库指纹", "题库版本", "独立测量版本", "漏接尝试", "未归类尝试"
  ]];
  const stageValues = (stage = {}) => [
    ...
    stage.max_score ?? "",
    stage.accuracy ?? "",
    Number(stage.pause_count || 0),
    Number(stage.paused_duration_ms || 0)
  ];
  safeRows(data).forEach((row) => {
    const pre = row.pretest || {};
    const formative = row.formative || {};
    const post = row.posttest || {};
    const courseware = row.courseware || {};
    const proactive = row.proactive || {};
    const retentionWindow = row.retention_window || {};
    const preExport = AdminPresentation.measurementExport(pre);
    const formativeExport = AdminPresentation.measurementExport(formative, { adaptive: true });
    const postExport = AdminPresentation.measurementExport(post);
    const observedRate = AdminPresentation.observedRate;
    const sameExperiment = protocol && protocol.experimentId === row.experiment_id;
    const definition = sameExperiment ? protocol.definition || {} : {};
    rows.push([
      row.nickname || "", row.user_id || "", row.experiment_id || "", row.condition || "",
      row.cohort || "", row.stratum || "", row.assignment_version || "", row.assigned_at || "",
      row.participation_status || "", row.consented_at || "", row.withdrawn_at || "",
      Number(row.current_learning_generation || 1), Number(row.analysis_learning_generation || 1),
      Number(row.learning_generation_count || 1),
      pre.submitted || 0, pre.expected || 0, preExport.score, preExport.maxScore, preExport.rate,
      preExport.status === "complete" ? pre.completed_at || "" : "",
      formative.submitted || 0, formative.expected || 0, formativeExport.score, formativeExport.maxScore,
      formativeExport.rate, formative.pending || 0, formative.completed_at || "",
      post.submitted || 0, post.expected || 0, postExport.score, postExport.maxScore,
      postExport.rate, post.pending || 0, postExport.status === "complete" ? post.completed_at || "" : "",
      courseware.semantic_events || 0, courseware.resources_opened || 0, courseware.scenes_completed || 0,
      courseware.challenge_count || 0, courseware.challenge_attempts || 0,
      courseware.challenge_correct_attempts || 0, observedRate(courseware.challenge_attempt_accuracy, courseware.challenge_attempts),
      courseware.first_independent_correct || 0, observedRate(courseware.first_independent_accuracy, courseware.challenge_count),
      courseware.independent_attempts || 0, courseware.independent_correct || 0,
      observedRate(courseware.independent_accuracy, courseware.independent_attempts),
      courseware.supported_attempts || 0, courseware.supported_correct || 0,
      observedRate(courseware.supported_accuracy, courseware.supported_attempts),
      courseware.retry_challenges || 0, observedRate(courseware.retry_rate, courseware.challenge_count), courseware.hint_events || 0,
      courseware.reset_events || 0, courseware.time_on_task_ms || 0,
      proactive.decisions || 0, proactive.policy_interventions || 0, proactive.delivered_interventions || 0,
      proactive.shown || 0, proactive.accepted || 0, proactive.dismissed || 0, proactive.ignored || 0,
      proactive.snoozed || 0, proactive.alternative_requested || 0, proactive.scope_mutes || 0,
      proactive.scope_restores || 0, proactive.l1 || 0, proactive.l2 || 0, proactive.l3 || 0,
      proactive.l4 || 0, proactive.l5 || 0,
      ...stageValues(row.outcomes?.near_transfer),
      ...stageValues(row.outcomes?.representation_transfer),
      ...stageValues(row.outcomes?.retention),
      retentionWindow.status || "", retentionWindow.unlock_at || "", retentionWindow.close_at || "",
      retentionWindow.elapsed_hours ?? "",
      safeRows(retentionWindow.shown_reminder_hours).join("|"),
      safeRows(retentionWindow.due_reminder_hours).join("|"),
      retentionWindow.closing_soon === true ? 1 : 0,
      retentionWindow.expired === true ? 1 : 0,
      row.data_quality?.status || "", row.data_quality?.issue_count || 0,
      safeRows(row.data_quality?.flags).join("|"), safeRows(row.learning_generations).join("|"),
      preExport.status, preExport.pending, formativeExport.status, postExport.status,
      sameExperiment ? protocol.fingerprint || "" : "",
      definition.courseAssessmentFingerprint || "", definition.courseAssessmentVersion || "",
      definition.outcomeInstrumentVersion || "", courseware.missed_attempts || 0,
      courseware.unclassified_attempts || 0
    ]);
  });
  return rows;
}

function proactiveChallengeAnalysisCsvRows(data) {
  const rate = AdminPresentation.observedRate;
  const rows = [[
    "知识点ID", "知识点", "挑战ID", "课件资源ID", "课件标题", "场景类型", "学生数", "尝试数",
    "正确尝试", "总尝试正确率%", "首次独立正确", "首次独立正确率%",
    "独立尝试", "独立正确", "独立正确率%", "提示或重置后尝试", "提示或重置后正确", "提示或重置后正确率%",
    "重试学生", "重试率%", "使用提示学生", "重置学生", "重置率%",
    "挑战完成学生", "挑战完成率%", "场景完成学生", "场景完成率%",
    "元数据问题", "重复事件", "顺序异常", "缺失事件链", "漏接尝试", "未归类尝试"
  ]];
  safeRows(data).forEach((row) => {
    rows.push([
      row.knowledge_point_id || "", row.knowledge_point || "", row.challenge_id || "",
      row.resource_id || "", row.resource_title || "", row.scene_type || "",
      row.participants || 0, row.attempts || 0, row.correct_attempts || 0,
      rate(row.total_attempt_accuracy, row.attempts), row.first_independent_correct || 0,
      rate(row.first_independent_accuracy, row.participants), row.independent_attempts || 0,
      row.independent_correct || 0, rate(row.independent_accuracy, row.independent_attempts),
      row.supported_attempts || 0, row.supported_correct || 0, rate(row.supported_accuracy, row.supported_attempts),
      row.retry_participants || 0, rate(row.retry_rate, row.participants), row.hint_participants || 0,
      row.reset_participants || 0, rate(row.reset_rate, row.participants),
      row.challenge_completed_participants || 0, rate(row.challenge_completion_rate, row.participants),
      row.scene_completed_participants || 0, rate(row.scene_completion_rate, row.participants),
      row.metadata_issues || 0, row.duplicate_events || 0, row.order_anomalies || 0,
      row.missing_event_chains || 0, row.missed_attempts || 0, row.unclassified_attempts || 0
    ]);
  });
  return rows;
}
...
function regradeAuditCsvRows(data) {
  const rows = [["审计ID", "批次ID", "状态", "学生", "用户ID", "题目ID", "单元ID", "评分提供方", "评分模型", "原评分", "建议评分", "实际应用", "错误信息", "时间",
    "请求模型", "服务端报告模型", "评分提示版本", "系统提示摘要", "评分量规摘要", "逐项评分"]];
  safeRows(data).forEach((row) => {
    let proposed = {};
    try { proposed = JSON.parse(row.proposed_grade_json || "{}") || {}; } catch {}
    ...
    audit.promptHash || "",
    audit.rubricHash || "",
    Array.isArray(proposed.rubricScores) ? JSON.stringify(proposed.rubricScores) : ""
  ]);
  });
  return rows;
}
...
function phaseCsvRows(data) {
  ...
  safeRows(data).forEach((row) => {
    const gain = row.pre_count > 0 && row.post_count > 0
      && !Number(row.pre_pending || 0) && !Number(row.post_pending || 0)
      && row.pre_accuracy != null && row.post_accuracy != null
...
document.getElementById("export-proactive-decisions-csv")?.addEventListener("click", (event) => {
  runCsvExport(event.currentTarget, async () => ({
    filename: `V14-C3主动政策决策账本-${exportDateStamp()}.csv`,
    rows: proactiveDecisionCsvRows(await fetchAllProactiveDecisionRows())
  }));
});

document.getElementById("export-proactive-shadow-csv")?.addEventListener("click", (event) => {
  runCsvExport(event.currentTarget, async () => ({
    filename: `V14-C3-Shadow双人编码模板-${exportDateStamp()}.csv`,
    rows: proactiveShadowCodingCsvRows(await fetchAllProactiveDecisionRows())
  }));
});

document.getElementById("export-proactive-outcomes-csv")?.addEventListener("click", (event) => {
  runCsvExport(event.currentTarget, async () => ({
    filename: `V14-C3独立学习结果-${exportDateStamp()}.csv`,
    rows: proactiveOutcomeCsvRows(await fetchAllProactiveOutcomeRows())
  }));
});

document.getElementById("export-proactive-outcome-sessions-csv")?.addEventListener("click", (event) => {
  runCsvExport(event.currentTarget, async () => ({
    filename: `V14-C3测量会话与失访-${exportDateStamp()}.csv`,
    rows: proactiveOutcomeSessionCsvRows(
      await fetchAllProactiveOutcomeSessionRows()
    )
  }));
});

document.getElementById("export-proactive-participants-csv")?.addEventListener("click", (event) => {
  runCsvExport(event.currentTarget, async () => ({
    filename: `V14-C3参与者实验分析-${exportDateStamp()}.csv`,
    rows: proactiveParticipantAnalysisCsvRows(
      cachedProactiveStudyData.participantAnalysis, cachedProactiveStudyData.protocol
    )
  }));
});

document.getElementById("export-research-analysis-package")?.addEventListener("click", async (event) => {
  const button = event.currentTarget;
  ...
  button.textContent = "正在导出...";
  try {
    const experimentId = cachedProactiveStudyData.protocol?.experimentId
      || cachedProactiveStudyData.policy?.experimentId || "";
    const params = new URLSearchParams({ experimentId });
    const data = await adminApi(`/api/admin/research/analysis-package?${params}`);
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `实验分析包-${exportDateStamp()}.json`;
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
    setTimeout(() => URL.revokeObjectURL(url), 0);
  } catch (error) {
    window.alert(`分析包导出失败：${error.message || "请稍后重试。"}`);
  } finally {
    button.disabled = false;
    button.removeAttribute("aria-busy");
...
document.getElementById("export-proactive-challenges-csv")?.addEventListener("click", (event) => {
  runCsvExport(event.currentTarget, async () => ({
    filename: `V14-C3课件挑战分析-${exportDateStamp()}.csv`,
    rows: proactiveChallengeAnalysisCsvRows(cachedProactiveStudyData.coursewareChallenges)
  }));
});
